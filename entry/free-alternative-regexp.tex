...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L14-L15}
...
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L17-L17}
...
else}. There's no structure or properties at \emph{all} about
\texttt{RegExp}\ldots{}other that it contains \texttt{Prim} and the minimal
structure/scaffolding to be an \texttt{Alternative} instance.

After adding some convenient wrappers\ldots{}we're done here!

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L19-L30}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L32-L35}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L37-L40}
...
\texttt{asum\ ::\ {[}Maybe\ a{]}\ -\textgreater{}\ Maybe\ a} finds the first
\texttt{Just} (success) in a list of attempts.
...
\OtherTok{matchChain ::} \DataTypeTok{AltF} \DataTypeTok{Prim}\NormalTok{ a }\OtherTok{->} \DataTypeTok{String} \OtherTok{->} \DataTypeTok{Maybe}\NormalTok{ a}
\NormalTok{matchChain (}\DataTypeTok{Ap}\NormalTok{ _          _   ) []     }\FunctionTok{=} \DataTypeTok{Nothing}
\NormalTok{matchChain (}\DataTypeTok{Ap}\NormalTok{ (}\DataTypeTok{Prim}\NormalTok{ c x) next) (d}\FunctionTok{:}\NormalTok{ds)}
    \FunctionTok{|}\NormalTok{ c }\FunctionTok{==}\NormalTok{ d    }\FunctionTok{=}\NormalTok{ matchAlts ((}\FunctionTok{$}\NormalTok{ x) }\FunctionTok{<$>}\NormalTok{ next) ds}
    \FunctionTok{|} \FunctionTok{otherwise} \FunctionTok{=} \DataTypeTok{Nothing}
...
\texttt{matchAlts} will match the \emph{prefix} of the string, so we need to try
all successive prefixes on an input string until we get a match.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L65-L66}

\OtherTok{match2 ::} \DataTypeTok{RegExp}\NormalTok{ a }\OtherTok{->} \DataTypeTok{String} \OtherTok{->} \DataTypeTok{Maybe}\NormalTok{ a}
\NormalTok{match2 l xs }\FunctionTok{=}\NormalTok{ asum [ matchAlts l ys }\FunctionTok{|}\NormalTok{ ys }\OtherTok{<-}\NormalTok{ tails xs ]}
\end{Highlighting}
\end{Shaded}
