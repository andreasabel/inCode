...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L20-L32}
...
\NormalTok{charAs c x }\FunctionTok{=}\NormalTok{ liftAlt (}\DataTypeTok{Prim}\NormalTok{ c x)     }\CommentTok{-- liftAlt lets us use the underlying}
                                    \CommentTok{-- functor Prim in RegExp, analogous}
                                    \CommentTok{-- to liftFM from earlier}
...
\texttt{put\ "hello"} does not correspond to \emph{any} regular expression.
...
parses a string based on a \texttt{RegExp}. To do this, we can pattern match and
handle the cases.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L64-L65}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/regexp.hs#L67-L72}
...
\DataTypeTok{Just} \DecValTok{3}
\NormalTok{ghci}\FunctionTok{>}\NormalTok{ matchAlts testRegexp }\StringTok{"acdcdcdcdcdcdcde"}
\DataTypeTok{Just} \DecValTok{7}
\end{Highlighting}
\end{Shaded}
