...
Our GADT here says that the two ``primitive'' commands of \texttt{State\ Int}
are ``putting'' (which requires an \texttt{Int} and produces a \texttt{()}
result) and ``getting'' (which requires no inputs, and produces an \texttt{Int}
result).
