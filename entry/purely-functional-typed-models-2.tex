...
\textbackslash ldots\}(x,t) = \textbackslash epsilon\_t + \textbackslash phi\_1
\textbackslash text\{AR\}\_\{\textbackslash phi\_1, \textbackslash phi\_2,
...
\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{Backprop}\NormalTok{ p, }\DataTypeTok{Backprop}\NormalTok{ s)}
    \OtherTok{=>} \DataTypeTok{ModelS}\NormalTok{  p    s  a b}
    \OtherTok{{-}>} \DataTypeTok{Model}\NormalTok{  (p }\OperatorTok{:\&}\NormalTok{ s) a b}
...
\NormalTok{ghci}\OperatorTok{>}\NormalTok{ series }\OtherTok{=}\NormalTok{ [ }\FunctionTok{sin}\NormalTok{ (}\DecValTok{2} \OperatorTok{*} \FunctionTok{pi} \OperatorTok{*}\NormalTok{ t }\OperatorTok{/} \DecValTok{25}\NormalTok{) }\OperatorTok{|}\NormalTok{ t }\OtherTok{<{-}}\NormalTok{ [}\DecValTok{0}\OperatorTok{..}\NormalTok{]              ]}

\CommentTok{{-}{-} chunks of runs and "next results"}
\NormalTok{ghci}\OperatorTok{>}\NormalTok{ samps  }\OtherTok{=}\NormalTok{ [ (}\FunctionTok{init}\NormalTok{ c, }\FunctionTok{last}\NormalTok{ c)      }\OperatorTok{|}\NormalTok{ c }\OtherTok{<{-}}\NormalTok{ chunksOf }\DecValTok{19}\NormalTok{ series ]}

\CommentTok{{-}{-} first layer is RNN, second layer is normal ANN, 30 hidden units}
\NormalTok{ghci}\OperatorTok{>} \KeywordTok{let}\OtherTok{ rnn ::} \DataTypeTok{ModelS}\NormalTok{ \_ \_ (}\DataTypeTok{R} \DecValTok{1}\NormalTok{) (}\DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{          rnn }\OtherTok{=}\NormalTok{ feedForward }\OperatorTok{@}\DecValTok{30} \OperatorTok{<*\textasciitilde{}}\NormalTok{ mapS logistic fcrnn}

\NormalTok{ghci}\OperatorTok{>}\NormalTok{ trained }\OtherTok{<{-}}\NormalTok{ trainModelIO (zeroState (unrollLast rnn)) }\OperatorTok{$} \FunctionTok{take} \DecValTok{10000}\NormalTok{ samps}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{prime f p }\OtherTok{=}\NormalTok{ foldl\textquotesingle{} }\OperatorTok{$}\NormalTok{ evalBP2 (\textbackslash{}s x }\OtherTok{{-}>} \FunctionTok{snd} \OperatorTok{$}\NormalTok{ f (auto p) x s)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\OperatorTok{>} \KeywordTok{let}\NormalTok{ primed }\OtherTok{=}\NormalTok{ prime    rnn trained }\DecValTok{0}\NormalTok{      (}\FunctionTok{take} \DecValTok{19}\NormalTok{ series)}
\NormalTok{ghci}\OperatorTok{>} \KeywordTok{let}\NormalTok{ output }\OtherTok{=}\NormalTok{ feedback rnn trained primed (series }\OperatorTok{!!} \DecValTok{19}\NormalTok{)}
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{mapM\_} \FunctionTok{print} \OperatorTok{$} \FunctionTok{take} \DecValTok{200}\NormalTok{ output}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\OperatorTok{>}\NormalTok{ trained }\OtherTok{<{-}}\NormalTok{ trainModelIO (zeroState (unrollLast ar2)) }\OperatorTok{$} \FunctionTok{take} \DecValTok{10000}\NormalTok{ samps}
\NormalTok{ghci}\OperatorTok{>} \KeywordTok{let}\NormalTok{ primed }\OtherTok{=}\NormalTok{ prime    rnn trained }\DecValTok{0}\NormalTok{      (}\FunctionTok{take} \DecValTok{19}\NormalTok{ series)}
\NormalTok{ghci}\OperatorTok{>} \KeywordTok{let}\NormalTok{ output }\OtherTok{=}\NormalTok{ feedback rnn trained primed (series }\OperatorTok{!!} \DecValTok{19}\NormalTok{)}
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{mapM\_} \FunctionTok{print} \OperatorTok{$} \FunctionTok{take} \DecValTok{200}\NormalTok{ output}
\NormalTok{(}\OperatorTok{{-}}\FloatTok{0.9980267284282716}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
