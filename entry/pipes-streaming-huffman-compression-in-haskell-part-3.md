...
(Originally posted by Justin Le [/] on June 26, 2014)
...
The choice between *conduit* and *pipes* depends a lot on what you want
...
(Remember that you can download \[encode.hs\]\[\] from github and try it
out yourself; just remember to also grab \[Huffman.hs\]\[\],
\[PQueue.hs\]\[\], and \[PreTree.hs\]\[\], and \[Weighted.hs\]\[\] from
the previous parts of this tutorial!)

!!\[encode.hs\]:huffman/encode.hs !!\[Huffman.hs\]:huffman/Huffman.hs
!!\[PQueue.hs\]:huffman/PQueue.hs !!\[PreTree.hs\]:huffman/PreTree.hs
...
This spells trouble for us, because our pipe will be merrily be waiting
...
Let’s take a look at the `dirsBytesP` parser, which parses `Direction`s
into a `Word8`:
...
Once we have our `pipeline`, we use `runEffect` to “run” it; then…that’s
it!
...
(Remember again that download \[decode.hs\]\[\] is also available online
from github! Again, be sure to also grab \[Huffman.hs\]\[\],
\[PQueue.hs\]\[\], and \[PreTree.hs\]\[\], and \[Weighted.hs\]\[\] from
posts past.)
...
2.  A component consuming the incoming `ByteString`s, and writing them
    to our output file.
...
It uses the *bits* package to turn an incoming `Word8` into a list of
...
Why `Consumer'` and not `Consumer`? Well, remember that all lines of the
