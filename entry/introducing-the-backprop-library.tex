...
A good first step in using \emph{backprop} is to create a nice custom data type
...
These are ways to \emph{access} components of our data type:
...
At this point, we are supposed to compute the \emph{gradient} of our error
...
produce a \texttt{Net} whose components contain the derivative of each component
with respect to the error. It tells us how to ``nudge'' each component to
increase the error function. \emph{Training} a neural network involves moving in
the opposite direction of the gradient, which causes the error to go
\emph{down}.
...
\texttt{norm\_1} (as \texttt{norm\_1V}) lifted to work with \texttt{BVar}s:
...
Both of these are are 100\% lexicographically \emph{identical} in implementation
to our original ones -- the only difference is that
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L88-L92}
...
\item
  To wrap it all up, we can ``scale'' our gradients because I added a
  \texttt{Num} and \texttt{Fractional} instance for \texttt{Net} while you
  weren't looking, using
  \emph{\href{http://hackage.haskell.org/package/one-liner-instances}{one-liner-instances}}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L143-L155}
...
complex the code you are backpropagating is. The overhead comes from the
building of the function call graph, but also potentially from the mechanical
automatic differentiation process generating different operations than what you
might write by hand. See the \href{https://github.com/mstksg/backprop}{README}
for a deeper analysis.

As you might have noticed from the RankN type signature (the
\texttt{forall\ s.\ ...}), \emph{backprop} uses the RankN type trick that
\texttt{Control.Monad.ST} and the
...
\href{https://github.com/mstksg/backprop}{README} has a deeper discussion on
this.

Writing a \texttt{Num} instance for your types is some manageable boilerplate if
your type derives Generic (so we can use
\emph{\href{http://hackage.haskell.org/package/one-liner-instances}{one-liner-instances}}),
...
\texttt{BVar}s. The library does provide lens-based accessors/setters, but, more
...
going as far as re-using our entire \texttt{logistic} implementation because it
...
other posts I'll be writing on applying \emph{backprop} to optimize things other
