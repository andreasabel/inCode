...
Tutorial} for beginners, to learn digit recognition using the MNIST data set. To
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-normal.hs#L8-L9}
...
The first step in using \emph{backprop} is to create a nice custom data type
containing all of the items you want to compute your gradients on:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-normal.hs#L11-L16}
...
\includegraphics{https://latex.codecogs.com/png.latex?\%5Cmathbb\%7BR\%7D\%5E\%7B784\%7D\%20\%5Crightarow\%20\%5Cmathbb\%7BR\%7D\%5E\%7B250\%7D}.
An \texttt{R\ 250} is a 250-vector, etc.

From the \emph{lens} library, four lenses are generated:
...
Which are ways to \emph{access} components of our data type:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-normal.hs#L34-L41}
...
is the matrix-vector multiplication operator from \emph{hmatrix}; we can also
just use \texttt{+} (from \texttt{Num}) to add vectors together.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-normal.hs#L18-L26}
...
We can define the logistic function using only \texttt{Num} operations.
\texttt{softMax} requires us to \texttt{norm\_1} (to get the absolute sum of all
items in a vector) from \emph{hmatrix}, and also \texttt{konst} (to generate a
vector of a single item repeated). Still, though, pretty much a straightforward
implementation of the mathematical definitions.
...
need a scalar \emph{error function} to minimize. This is a function on the
network that, given an input the expected output, computes how ``bad'' the
network is. It computes the error between the output of the network and the
expected output, as a single number.

To do this, we will be using the
\href{https://en.wikipedia.org/wiki/Cross_entropy}{cross entropy} between the
target output and the network output. This is a pretty standard error function
for classification problems, and smaller cross-entropies indicate better
predictions.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-normal.hs#L28-L48}
...
At this point, we are meant to compute the \emph{gradient} of our error
function. It's a function that computes the \emph{direction of greatest change}
of all of the components in our network, with respect to an error function.

The gradient will take our \texttt{Net\ -\textgreater{}\ Double} function and
produce a \texttt{Net}, whose components contain the derivative of each
component with respect to the output. It tells us how to ``nudge'' each
component to increase the error function. \emph{Training} a neural network
involves moving in the opposite direction of the gradient, which causes the
error to go \emph{down}.
...
into the world of \emph{backprop} and get automatic differentiation!

\hypertarget{with-backprop}{%
\section{With Backprop}\label{with-backprop}}

Now let's see what happens if we compute our error function using
\emph{backprop}, instead!
...
We add \texttt{Numeric.Backprop}, the module where the magic happens.

We switch from \texttt{Numeric.LinearAlgebra.Static} to
...
type slightly -- instead of working directly with values of type \texttt{a}, we
work with \texttt{BVar}s (backpropagatable variables) \emph{containing}
\texttt{a}s: a \texttt{BVar\ s\ a}.

For example, let's look at our updated \texttt{softMax}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L55-L61}
...
But, \texttt{Numeric.LinearAlgebra.Static.Backprop} re-exports \texttt{konst}
and \texttt{norm\_1} (as \texttt{norm\_1V}) lifted to work with \texttt{BVar}s:
...
And, \texttt{BVar}s have \texttt{Num}, \texttt{Fractional}, and
\texttt{Floating} instances, so \texttt{exp} and \texttt{/} already work
out-of-the-box.

Now, \texttt{softMax} is automatically differentiable!

Note that, because of \texttt{BVar}'s numeric instances, we can re-use our
original implementation of \texttt{logistic}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L52-L53}
...
Now, to \emph{run} our network, things look pretty similar:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L70-L78}
...
using \texttt{\^{}.} for lens access, we use \texttt{\^{}\^{}.}, for lens access
into a \texttt{BVar}:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{myNetVar                ::} \DataTypeTok{BVar}\NormalTok{ s }\DataTypeTok{Net}          \CommentTok{-- a Net inside a BVar}
\NormalTok{myNetVar }\FunctionTok{^^.}\OtherTok{ weights1   ::} \DataTypeTok{BVar}\NormalTok{ s (}\DataTypeTok{L} \DecValTok{250} \DecValTok{784}\NormalTok{)  }\CommentTok{-- access the weights1 field in myNet}
\NormalTok{myNetVar }\FunctionTok{^^.}\OtherTok{ bias2      ::} \DataTypeTok{BVar}\NormalTok{ s (}\DataTypeTok{R}  \DecValTok{10}\NormalTok{    )  }\CommentTok{-- access the bias2 field in myNet}
\end{Highlighting}
\end{Shaded}
...
Using lenses like this gives us a virtually frictionless usage of
...
instead of \texttt{.\textasciitilde{}}, and even access constructors in sum
types using \texttt{\^{}\^{}?} (which can implement pattern matching) and get
matches for \emph{multiple} targets using \texttt{\^{}\^{}..}:
...
\emph{backprop} \texttt{runNet} is as mechanical as could be.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L63-L86}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L88-L95}
...
\end{Shaded}
  ...
  If we ignore the existential type/\texttt{Reifies} syntax noise, this can be
  read as:
...
\item
  ...
  error function, or \texttt{BVar\ s\ Net\ -\textgreater{}\ BVar\ s\ Double}.
  That's exactly what \texttt{netErr} gives us.
...
\end{Shaded}
  ...
  Which we can pass to \texttt{gradBP} to our the gradient of the network
  \texttt{Net} with respect to the \texttt{Double} error.
\end{enumerate}

Would you believe me if I told you that was it?

Note that we can ``scale'' our network like that because I added a \texttt{Num}
and \texttt{Fractional} instance for \texttt{Net} without telling you, using
\emph{\href{http://hackage.haskell.org/package/one-liner-instances}{one-liner-instances}}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/backprop/intro-backprop.hs#L146-L158}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\ExtensionTok{./intro-backprop}\NormalTok{ PATH_TO_DATA/\{train-images-idx3-ubyte,\textbackslash{}}
\NormalTok{    train-labels-idx1-ubyte,\textbackslash{}}
\NormalTok{    t10k-images-idx3-ubyte,\textbackslash{}}
\NormalTok{    t10k-labels-idx1-ubyte\}}
...
\hypertarget{behind-the-magic}{%
\section{Behind the Magic}\label{behind-the-magic}}
...
\texttt{BVar\ s\ a\ -\textgreater{}\ BVar\ s\ b} function, and using one of the
three runners:
...
\texttt{evalBP} carries virtually zero performance overhead (about 4\%) over
writing your functions directly, so there's pretty much no harm in writing your
entire application or library in \texttt{BVar}-based code. \texttt{gradBP}
carries a some measurable performance overhead over writing your gradient code
``manually'', but this heavily depends on exactly how complex the code you are
backpropagating is. The overhead comes from the building of the function call
graph, but also potentially from the mechanical automatic differentiation
process generating different operations than what you might write by hand.

\emph{backprop} uses the RankN type trick that \texttt{Control.Monad.ST} and the
\emph{\href{http://hackage.haskell.org/package/ad}{ad}} library does, for two
reasons:
...
\item
  ...
  prevents these variables from leaking out of the backprop world.
\item
  ...
  your computation, which it uses internally to perform the backpropagation.
  (The \texttt{W} stands for Wengert).
\end{enumerate}
...
\texttt{Num} instance on the things being backpropagated.

This is an API decision that is a compromise between different options. Really,
the only thing the library needs is \texttt{(+)}, \texttt{fromInteger\ 0}, and
(for only the final result type) \texttt{fromIntegral\ 1}. \texttt{Num} is
chosen because of how pervasive it already is in the Haskell ecosystem, but in
theory, it is definitely a bit more powerful than needed.

Writing a \texttt{Num} instance for your types is pretty boilerplate if your
type derives Generic (so we can use
\emph{\href{http://hackage.haskell.org/package/one-liner-instances}{one-liner-instances}}),
like we saw above with the \texttt{Num} instance for \texttt{Net}.

In practice, too, requiring a \texttt{Num} instance means you can't directly
...
\texttt{Numeric.Backprop.Tuple}, and if you are writing an application, you can
use the orphan instances in
...
\texttt{BVar}s. The library provides the lens-based accessors/setters, but, more
importantly, it gives \texttt{Num}, \texttt{Fractional}, and \texttt{Floating}
instances for \texttt{BVar}s so you can manipulate a \texttt{BVar\ s\ a} just
like an \texttt{a} using its numeric instances. We leveraged this heavily by
using \texttt{+}, \texttt{negate}, \texttt{log}, \texttt{/}, etc., and even
going as far as re-using our entire \texttt{logistic} definition because it only
...
their types and operations with \emph{backprop}. However, writing them is not
magical -- it just requires providing the result and the gradient with respect
to a final total derivative. Let's look at the implementation of
...
quick write-up} on how to apply those techniques to \emph{backprop} (also
available in
