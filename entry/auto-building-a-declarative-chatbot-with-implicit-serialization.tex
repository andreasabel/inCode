...
\NormalTok{perRoom rb }\OtherTok{=}\NormalTok{ proc inp}\OperatorTok{@}\NormalTok{(}\DataTypeTok{InMessage}\NormalTok{ \_ \_ src \_) }\OtherTok{{-}>} \KeywordTok{do}
\NormalTok{    messages }\OtherTok{<{-}}\NormalTok{ fromBlips [] }\OperatorTok{.}\NormalTok{ rb }\OperatorTok{{-}<}\NormalTok{ inp}
    \FunctionTok{id} \OperatorTok{{-}<} \DataTypeTok{OutMessages} \OperatorTok{$}\NormalTok{ M.singleton src messages}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{free-serialization}{%
...
\NormalTok{        ircconf\textquotesingle{} }\OtherTok{=}\NormalTok{ ircconf \{ cEvents }\OtherTok{=}\NormalTok{ events \}}
    ...
    \DataTypeTok{Right}\NormalTok{ server }\OtherTok{<{-}}\NormalTok{ connect ircconf\textquotesingle{} }\DataTypeTok{True} \DataTypeTok{True}
...
\NormalTok{      \_ }\OtherTok{<{-}} \FunctionTok{flip}\NormalTok{ M.traverseWithKey outs }\OperatorTok{$}\NormalTok{ \textbackslash{}channel messages }\OtherTok{{-}>} \KeywordTok{do}
...
\NormalTok{        forM\_ messages }\OperatorTok{$}\NormalTok{ \textbackslash{}message }\OtherTok{{-}>} \KeywordTok{do}
...
\NormalTok{    onMessage inputChan }\OtherTok{=}\NormalTok{ \textbackslash{}\_ message }\OtherTok{{-}>} \KeywordTok{do}
...
\NormalTok{    get }\OtherTok{=} \FunctionTok{read} \OperatorTok{<$>}\NormalTok{ get      }\CommentTok{{-}{-} haha don\textquotesingle{}t do this in real life.}
...
\NormalTok{    get }\OtherTok{=} \DataTypeTok{ModifiedJulianDay} \OperatorTok{<$>}\NormalTok{ get}
...
\NormalTok{seenBot }\OtherTok{=}\NormalTok{ proc (}\DataTypeTok{InMessage}\NormalTok{ nick msg \_ time) }\OtherTok{{-}>} \KeywordTok{do}
\NormalTok{    seens  }\OtherTok{<{-}}\NormalTok{ trackSeens }\OperatorTok{{-}<}\NormalTok{ (nick, time)}
...
\NormalTok{        respond qry }\OtherTok{=} \KeywordTok{case}\NormalTok{ M.lookup qry seens }\KeywordTok{of}
                        \DataTypeTok{Just}\NormalTok{ t  }\OtherTok{{-}>}\NormalTok{ [qry }\OperatorTok{++} \StringTok{" last seen at "} \OperatorTok{++} \FunctionTok{show}\NormalTok{ t }\OperatorTok{++} \StringTok{"."}\NormalTok{]}
                        \DataTypeTok{Nothing} \OtherTok{{-}>}\NormalTok{ [}\StringTok{"No record of "} \OperatorTok{++}\NormalTok{ qry }\OperatorTok{++} \StringTok{"."}\NormalTok{]}

    \FunctionTok{id} \OperatorTok{{-}<}\NormalTok{ respond }\OperatorTok{<$>}\NormalTok{ queryB}
  \KeywordTok{where}
\OtherTok{    trackSeens ::} \DataTypeTok{Monad}\NormalTok{ m }\OtherTok{=>} \DataTypeTok{Auto}\NormalTok{ m (}\DataTypeTok{Nick}\NormalTok{, }\DataTypeTok{UTCTime}\NormalTok{) (}\DataTypeTok{Map} \DataTypeTok{Nick} \DataTypeTok{UTCTime}\NormalTok{)}
\NormalTok{    trackSeens }\OtherTok{=}\NormalTok{ accum (\textbackslash{}mp (nick, time) }\OtherTok{{-}>}\NormalTok{ M.insert nick time mp) M.empty}
\OtherTok{    queryBlips ::} \DataTypeTok{Auto}\NormalTok{ m }\DataTypeTok{Message}\NormalTok{ (}\DataTypeTok{Blip} \DataTypeTok{Nick}\NormalTok{)}
...
\NormalTok{            rep }\OtherTok{=}\NormalTok{ M.findWithDefault }\DecValTok{0}\NormalTok{ nick reps}
    ...
    \FunctionTok{id} \OperatorTok{{-}<}\NormalTok{ lookupRep }\OperatorTok{<$>}\NormalTok{ queryB}
  \KeywordTok{where}
\OtherTok{    updateBlips ::} \DataTypeTok{Auto}\NormalTok{ m (}\DataTypeTok{Nick}\NormalTok{, }\DataTypeTok{Message}\NormalTok{) (}\DataTypeTok{Blip}\NormalTok{ (}\DataTypeTok{Nick}\NormalTok{, }\DataTypeTok{Int}\NormalTok{))}
\NormalTok{    updateBlips }\OtherTok{=}\NormalTok{ emitJusts getUpdateCommand}
      \KeywordTok{where}
...
\NormalTok{announceBot chans }\OtherTok{=}\NormalTok{ proc (}\DataTypeTok{InMessage}\NormalTok{ nick msg src time) }\OtherTok{{-}>} \KeywordTok{do}
\NormalTok{    announceB }\OtherTok{<{-}}\NormalTok{ announceBlips     }\OperatorTok{{-}<}\NormalTok{ (nick, msg)}

\NormalTok{    newDayB   }\OtherTok{<{-}}\NormalTok{ newDayBlips       }\OperatorTok{{-}<}\NormalTok{ utctDay time}

\NormalTok{    annCounts }\OtherTok{<{-}}\NormalTok{ resetOn trackAnns }\OperatorTok{{-}<}\NormalTok{ (nick }\OperatorTok{<$}\NormalTok{ announceB, newDayB)}

    \KeywordTok{let}\NormalTok{ hasFlooded  }\OtherTok{=}\NormalTok{ M.findWithDefault }\DecValTok{0}\NormalTok{ nick annCounts }\OperatorTok{>} \DecValTok{3}
...
\NormalTok{        outB        }\OperatorTok{|}\NormalTok{ hasFlooded }\OtherTok{=}\NormalTok{ [nick }\OperatorTok{++} \StringTok{": No flooding!"}\NormalTok{] }\OperatorTok{<$}\NormalTok{ announceB}
                    \OperatorTok{|} \FunctionTok{otherwise}  \OtherTok{=}\NormalTok{ announceB}

\OtherTok{        outMsgsB    ::} \DataTypeTok{Blip} \DataTypeTok{OutMessages}
\NormalTok{        outMsgsB    }\OtherTok{=}\NormalTok{ (\textbackslash{}out }\OtherTok{{-}>} \DataTypeTok{OutMessages}\NormalTok{ (M.fromList (}\FunctionTok{map}\NormalTok{ (,out) targetChans)))}
                  \OperatorTok{<$>}\NormalTok{ outB}
...
\OtherTok{chatBot\textquotesingle{} ::} \DataTypeTok{MonadIO}\NormalTok{ m }\OtherTok{=>} \DataTypeTok{ChatBot}\NormalTok{ m}
...
\OtherTok{isolatedRooms ::} \DataTypeTok{Monad}\NormalTok{ m }\OtherTok{=>} \DataTypeTok{RoomBot}\NormalTok{ m }\OtherTok{{-}>} \DataTypeTok{ChatBot}\NormalTok{ m}
\NormalTok{isolatedRooms rb }\OtherTok{=}\NormalTok{ proc inp}\OperatorTok{@}\NormalTok{(}\DataTypeTok{InMessage}\NormalTok{ \_ \_ src \_) }\OtherTok{{-}>} \KeywordTok{do}
\NormalTok{    messages }\OtherTok{<{-}}\NormalTok{ fromBlips [] }\OperatorTok{.}\NormalTok{ mux (}\FunctionTok{const}\NormalTok{ rb) }\OperatorTok{{-}<}\NormalTok{ (src, inp)}
    \FunctionTok{id} \OperatorTok{{-}<} \DataTypeTok{OutMessages} \OperatorTok{$}\NormalTok{ M.singleton src messages}
\end{Highlighting}
\end{Shaded}
