...
the new state is `x+1`…we can return whether or not it’s odd, in which case the
new state is `x-1`….or we can return whether or not it’s positive, in which case
the new state is `negate x`”
...
for when you took the first and the third….and the second and the first…etc.,
etc.
...
expressiveness. But not that it matters…the original version runs fast already.
