...
is\ldots still a lot of function compositions in the end.\footnote{We only
  ...
  \texttt{\textless{}\textgreater{}}, where both sides themselves point to the
  same function pointer{[}\^{}pointer{]}), so it's very efficient in space as
  well, but to actually ``run'' that final function, we need to still traverse
  all of those nested pointers the full number of times.} So, while composition
with \texttt{\textless{}\textgreater{}} is cheap, application with
