...
\CommentTok{{-}{-} | Given a permutation list, find the place where a given index ends up.}
\OtherTok{(@$) ::} \DataTypeTok{Perm}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n}
...
\NormalTok{part1 perms }\OtherTok{=}\NormalTok{ bigPerm }\OperatorTok{@$} \DecValTok{2019}
  \KeywordTok{where}
...
\NormalTok{part2 perms }\OtherTok{=}\NormalTok{ invert biiigPerm }\OperatorTok{@$} \DecValTok{2020}
  \KeywordTok{where}
\NormalTok{    bigPerm   }\OtherTok{=} \FunctionTok{mconcat}\NormalTok{ perms}
...
representation -- the definition of \texttt{Perm\ n}. A good first guess might
be to write our permutation as an actual function --- a function from index to
index, \texttt{Finite\ n\ -\textgreater{}\ Finite\ n}. Then, we can just use
function composition as our permutation composition.
...
\OtherTok{(@$) ::} \DataTypeTok{Perm}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n}
\DataTypeTok{Perm}\NormalTok{ f }\OperatorTok{@$}\NormalTok{ x  }\OtherTok{=}\NormalTok{ f x}
...
\OtherTok{(@$) ::} \DataTypeTok{KnownNat}\NormalTok{ n }\OtherTok{=>} \DataTypeTok{Affine}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n }\OtherTok{{-}>} \DataTypeTok{Finite}\NormalTok{ n}
\DataTypeTok{Aff}\NormalTok{ a b }\OperatorTok{@$}\NormalTok{ x }\OtherTok{=}\NormalTok{ a }\OperatorTok{*}\NormalTok{ x }\OperatorTok{+}\NormalTok{ b}
...
If we want to do
\includegraphics{https://latex.codecogs.com/png.latex?f\%28x\%29\%20\%3D\%20a\%27\%20x\%20\%2B\%20b}
after
\includegraphics{https://latex.codecogs.com/png.latex?g\%28x\%29\%20\%3D\%20a\%20x\%20\%2B\%20b},
it's:
...
\NormalTok{        b\textquotesingle{} }\OtherTok{=} \FunctionTok{negate} \OperatorTok{$}\NormalTok{ a\textquotesingle{} }\OperatorTok{*}\NormalTok{ b}
\end{Highlighting}
\end{Shaded}
