...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Justin Le},
            pdftitle={Practical Dependent Types in Haskell: Type-Safe Linear Algebra},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{readList}\OtherTok{ ::} \DataTypeTok{IO} \NormalTok{[}\DataTypeTok{Double}\NormalTok{]}
\NormalTok{readList }\FunctionTok{=} \NormalTok{concat }\FunctionTok{.} \NormalTok{readMaybe }\FunctionTok{<$>} \NormalTok{getLine}

\OtherTok{dotStdin ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{dotStdin }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{v1 }\OtherTok{<-} \NormalTok{vector }\FunctionTok{<$>} \NormalTok{readList}
    \NormalTok{v2 }\OtherTok{<-} \NormalTok{vector }\FunctionTok{<$>} \NormalTok{readList}
    \NormalTok{print }\FunctionTok{$} \NormalTok{l1 }\FunctionTok{<.>} \NormalTok{l2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{readChain ::} \DataTypeTok{IO} \NormalTok{[}\DataTypeTok{Matrix} \DataTypeTok{Double}\NormalTok{]}
\NormalTok{readChain }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{l }\OtherTok{<-} \NormalTok{getLine}
    \KeywordTok{case} \NormalTok{fromLists }\FunctionTok{<$>} \NormalTok{readMaybe l }\KeywordTok{of}
      \DataTypeTok{Just} \NormalTok{m  }\OtherTok{->} \NormalTok{(m}\FunctionTok{:}\NormalTok{) }\FunctionTok{<$>} \NormalTok{readChain}
      \DataTypeTok{Nothing} \OtherTok{->} \NormalTok{return []}
    
\OtherTok{collapseChain ::} \DataTypeTok{Vector} \DataTypeTok{Double} \OtherTok{->} \DataTypeTok{Matrix} \DataTypeTok{Double} \OtherTok{->} \DataTypeTok{Vector} \DataTypeTok{Double}
\NormalTok{collapseChain v []     }\FunctionTok{=} \NormalTok{v}
\NormalTok{collapseChain v (m}\FunctionTok{:}\NormalTok{ms) }\FunctionTok{=} \NormalTok{collapseChain (m }\FunctionTok{#>} \NormalTok{v) ms}

\OtherTok{chainMats ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{chainMats }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{ms }\OtherTok{<-} \NormalTok{readChain}
    \NormalTok{v  }\OtherTok{<-} \NormalTok{vector }\FunctionTok{<$>} \NormalTok{readList}
    \NormalTok{print }\FunctionTok{$} \NormalTok{collapseChain v ms}
\end{Highlighting}
\end{Shaded}
