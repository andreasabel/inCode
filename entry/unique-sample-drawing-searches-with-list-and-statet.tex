...
So \texttt{foo} takes a number, \texttt{x}, and says, ``here are three
ways we might proceed from having this number. We can return whether or
not it's even, in which case the new state is \texttt{x+1}\ldots{}we can
return whether or not it's odd, in which case the new state is
\texttt{x-1}\ldots{}.or we can return whether or not it's positive, in
which case the new state is \texttt{negate\ x}''

What the monad instance does is that it allows you to ``chain'' forks,
and go along different forks, and gather together ``all possible forks''
you could have taken. At the end, it outputs all possible forks. So if
you did \texttt{foo\ \textgreater{}\textgreater{}\ foo}, there'd be nine
results --- one result for when you took the first route (the
\texttt{x+1}) twice, one for when you took the first and then the second
(\texttt{x-1}), one for when you took the first and the
third\ldots{}.and the second and the first\ldots{}etc., etc.
...
which is a \texttt{StateT} action that says ``kill this current branch
if given \texttt{False}, or go on if given \texttt{True}''
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/send-more-money.hs#L3-35}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\KeywordTok{ghc} \NormalTok{-O2 send-more-money.hs}
\NormalTok{$ }\KeywordTok{./send-more-money}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/misc/send-more-money.hs#L38-59}
\OtherTok{select' ::} \NormalTok{[a] }\OtherTok{->} \NormalTok{[(a,[a])]}
