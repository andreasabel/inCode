...
\NormalTok{    isLeaf }\OtherTok{<{-}}\NormalTok{ get}
    \KeywordTok{if}\NormalTok{ isLeaf}
      \KeywordTok{then} \DataTypeTok{PTLeaf} \OperatorTok{<$>}\NormalTok{ get}
      \KeywordTok{else} \DataTypeTok{PTNode} \OperatorTok{<$>}\NormalTok{ get }\OperatorTok{<*>}\NormalTok{ get}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\OperatorTok{>}\NormalTok{ (}\OperatorTok{+}\NormalTok{) }\DecValTok{1} \DecValTok{4}
\DecValTok{5}
\NormalTok{ghci}\OperatorTok{>}\NormalTok{ (}\OperatorTok{+}\NormalTok{) }\OperatorTok{<$>} \DataTypeTok{Just} \DecValTok{1} \OperatorTok{<*>} \DataTypeTok{Just} \DecValTok{4}
...
\NormalTok{encodeAll pt xs }\OtherTok{=} \FunctionTok{concat} \OperatorTok{<$>} \FunctionTok{sequence}\NormalTok{ (}\FunctionTok{map}\NormalTok{ (lookupPTTable tb) xs)}
  \KeywordTok{where}
...
\OtherTok{decodeAll\textquotesingle{} ::} \DataTypeTok{PreTree}\NormalTok{ a }\OtherTok{{-}>} \DataTypeTok{Encoding} \OtherTok{{-}>} \DataTypeTok{Maybe}\NormalTok{ [a]}
