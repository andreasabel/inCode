...
\textbf{\href{https://blog.jle.im/entry/hamilton-dynamics-in-haskell.html}{in
Code}}.}
...
The system's \emph{Hamiltonian} is a \(\mathbb{R}^{2n} -> \mathbb{R}\) function
on the phase space (\(\mathbb{R}^{2n}\), where \(n\) is the number of
...
\href{/img/entries/hamilton/contour-lines.jpg}{Contour Lines}
...
monentum conjugate to that coordinate. \(\dot{q}\) is the rate of change of
\(q\), and \(\dot{p_q}\) is the rate of change of \(p_q\).
