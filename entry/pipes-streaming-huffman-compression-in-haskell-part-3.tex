...
(\texttt{(\textgreater{}-\textgreater{})}) doesn't work. We're going to
have to transform our \texttt{Direction} producer in another way.

Luckily for us, this is precisely the problem that \emph{pipes-parse}
was made to solve.

We'll go into more detail about just \emph{how} it solves this later. At
the high level, instead of composing pipes with
\texttt{(\textgreater{}-\textgreater{})}, we'll \emph{transform} pipes
by using pipe transformers/functions.

So we'll modify our plan. We'll have our ``\texttt{Direction}
producer'', which consists of:
...
And then we ``transform'' that \texttt{Direction} producer into a
\texttt{Word8} producer, which we'll call \texttt{dirsBytes}:
...
which turns a \texttt{Direction} producer into a \texttt{Word8} producer
that clumps up the \texttt{Direction}s into groups of 8 --- and if the
directions run out, pad the rest of the byte with 0's.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L19-46}
...
\KeywordTok{import qualified} \DataTypeTok{Data.ByteString.Lazy} \KeywordTok{as} \DataTypeTok{BL}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L63-74}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L78-92}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L108-117}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L123-137}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/encode.hs#L86-86}
          \NormalTok{bsOut     }\FunctionTok{=} \NormalTok{view PB.pack }\FunctionTok{.} \NormalTok{dirsBytes }\FunctionTok{$} \NormalTok{dirsOut}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\KeywordTok{ghc} \NormalTok{-O2 encode.hs}
\NormalTok{$ }\KeywordTok{./encode} \NormalTok{warandpeace.txt warandpeace.enc}
\NormalTok{$ }\KeywordTok{du} \NormalTok{-h warandpeace.*}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/decode.hs#L48-69}
...
\emph{too} necessary, but doing this will expose us to some nice
\emph{pipes} idioms :)

One way we can do it is to turn \texttt{searchPT} into a
\texttt{Consumer\textquotesingle{}} (a \texttt{Consumer} with the ends
not sealed off) that consumes \texttt{Direction}s and \emph{returns}
resulting \texttt{Word8}s.

Then we use \texttt{(\textgreater{}\textasciitilde{}\ cat)}, which turns
a \texttt{Consumer\textquotesingle{}} into something that is forever
consuming and re-yielding --- in essence, it turns a
\texttt{Consumer\textquotesingle{}} returning values into a
\texttt{Pipe} repeatedly yielding the returned values.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/huffman/decode.hs#L74-86}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\KeywordTok{ghc} \NormalTok{-O2 decode.hs}
\NormalTok{$ }\KeywordTok{./decode} \NormalTok{warandpeace.enc warandpeace.dec}
\NormalTok{$ }\KeywordTok{md5sum} \NormalTok{warandpeace.txt}
\CommentTok{# 3c8168e48f49784ac3c2c25d15388e96  warandpeace.txt}
\NormalTok{$ }\KeywordTok{md5sum} \NormalTok{warandpeace.dec}
