...
\OtherTok{analyzeFile ::} \DataTypeTok{FilePath} \OtherTok{{-}>} \DataTypeTok{IO}\NormalTok{ (}\DataTypeTok{Maybe}\NormalTok{ (}\DataTypeTok{Int}\NormalTok{, }\DataTypeTok{PreTree} \DataTypeTok{Word8}\NormalTok{))}
\NormalTok{analyzeFile fp }\OtherTok{=}\NormalTok{ withFile fp }\DataTypeTok{ReadMode} \OperatorTok{$}\NormalTok{ \textbackslash{}hIn }\OtherTok{{-}>} \KeywordTok{do}
...
\NormalTok{        tree }\OtherTok{=}\NormalTok{ evalState (listQueueStateTable fqs }\OperatorTok{>>}\NormalTok{ buildTree) emptyPQ}
...
\OtherTok{encodeFile ::} \DataTypeTok{FilePath} \OtherTok{{-}>} \DataTypeTok{FilePath} \OtherTok{{-}>} \DataTypeTok{Int} \OtherTok{{-}>} \DataTypeTok{PreTree} \DataTypeTok{Word8} \OtherTok{{-}>} \DataTypeTok{IO}\NormalTok{ ()}
\NormalTok{encodeFile inp out len tree }\OtherTok{=}
\NormalTok{    withFile inp }\DataTypeTok{ReadMode}  \OperatorTok{$}\NormalTok{ \textbackslash{}hIn  }\OtherTok{{-}>}
\NormalTok{    withFile out }\DataTypeTok{WriteMode} \OperatorTok{$}\NormalTok{ \textbackslash{}hOut }\OtherTok{{-}>} \KeywordTok{do}
\NormalTok{      BL.hPut hOut }\OperatorTok{$}\NormalTok{ encode (len, tree)}
      \KeywordTok{let}\NormalTok{ dirsOut   }\OtherTok{=}\NormalTok{ PB.fromHandle hIn}
                  \OperatorTok{>{-}>}\NormalTok{ bsToBytes}
                  \OperatorTok{>{-}>}\NormalTok{ encodeByte encTable}
...
\NormalTok{          pipeline  }\OtherTok{=}\NormalTok{ bsOut}
                  \OperatorTok{>{-}>}\NormalTok{ PB.toHandle hOut}
...
\NormalTok{    (result, leftovers) }\OtherTok{<{-}}\NormalTok{ lift }\OperatorTok{$}\NormalTok{ runStateT dirsBytesP p}
...
\NormalTok{        dirsBytes leftovers}
      \DataTypeTok{Nothing}   \OtherTok{{-}>} \FunctionTok{return}\NormalTok{ ()}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{    isEnd }\OtherTok{<{-}}\NormalTok{ isEndOfInput}
    \KeywordTok{if}\NormalTok{ isEnd}
      ...
      \KeywordTok{else} \DataTypeTok{Just} \OperatorTok{<$>}\NormalTok{ go }\DecValTok{0} \DecValTok{0}
  \KeywordTok{where}
...
\NormalTok{      dir }\OtherTok{<{-}}\NormalTok{ draw}
      \KeywordTok{case}\NormalTok{ dir }\KeywordTok{of}
        \DataTypeTok{Just} \DataTypeTok{DLeft}  \OtherTok{{-}>}\NormalTok{ go     b            (i }\OperatorTok{+} \DecValTok{1}\NormalTok{)}
        \DataTypeTok{Just} \DataTypeTok{DRight} \OtherTok{{-}>}\NormalTok{ go     (setBit b i) (i }\OperatorTok{+} \DecValTok{1}\NormalTok{)}
        \DataTypeTok{Nothing}     \OtherTok{{-}>} \FunctionTok{return}\NormalTok{ b}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{decodeFile ::} \DataTypeTok{FilePath} \OtherTok{{-}>} \DataTypeTok{FilePath} \OtherTok{{-}>} \DataTypeTok{IO}\NormalTok{ ()}
...
\NormalTok{      (metadata, decodingPipe) }\OtherTok{<{-}}\NormalTok{ runStateT PB.decode metadataPipe}
      ...
      \KeywordTok{case}\NormalTok{ metadata }\KeywordTok{of}
        ...
        \DataTypeTok{Right}\NormalTok{ (len, tree) }\OtherTok{{-}>} \KeywordTok{do}
          ...
          \KeywordTok{let}\NormalTok{ bytesOut  }\OtherTok{=}\NormalTok{ decodingPipe }\OperatorTok{>{-}>}\NormalTok{ bsToBytes}
...
\NormalTok{searchPT t }\OtherTok{=}\NormalTok{ searchPT\textquotesingle{} t }\OperatorTok{>\textasciitilde{}}\NormalTok{ cat}
  \KeywordTok{where}
\OtherTok{    searchPT\textquotesingle{} ::} \DataTypeTok{PreTree}\NormalTok{ a }\OtherTok{{-}>} \DataTypeTok{Consumer\textquotesingle{}} \DataTypeTok{Direction}\NormalTok{ m a}
...
\NormalTok{bsOut     }\OtherTok{=} \FunctionTok{flip}\NormalTok{ (over PB.unpack) bsIn }\OperatorTok{$}\NormalTok{ \textbackslash{}bytesOut }\OtherTok{{-}>}
