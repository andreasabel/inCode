...
(\texttt{s\ \textless{}\textasciitilde{}\textgreater{}\ a}), then we can use:
...
\texttt{s\ \textless{}\textasciitilde{}\textgreater{}\ a}, we can transform a
\texttt{p\ a\ a} into a \texttt{p\ s\ s} (a relationship on \texttt{a}s to be a
relationship on \texttt{s}).
...
You can think of it as taking a ``relationship on \texttt{a}is'' and turning it
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{first' ::} \DataTypeTok{Strong}\NormalTok{ p }\OtherTok{=>}\NormalTok{ p a a }\OtherTok{->}\NormalTok{ p (a, q) (a, q)}
\end{Highlighting}
\end{Shaded}
...
\texttt{p\ a\ a} to a \texttt{p\ s\ s}. And since
\texttt{s\ \textless{}\textasciitilde{}\textgreater{}\ (a,\ q)}, it promotes a
relationship on a ``part'' to be a relationship on a ``whole''. A lens!
...
You can also think of this as taking a ``relationship on \texttt{a}is'' and
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{left ::} \DataTypeTok{Choice}\NormalTok{ p }\OtherTok{=>}\NormalTok{ p a a }\OtherTok{->}\NormalTok{ p (}\DataTypeTok{Either}\NormalTok{ a q) (}\DataTypeTok{Either}\NormalTok{ a q)}
\end{Highlighting}
\end{Shaded}
...
\texttt{p\ a\ a} to a \texttt{p\ s\ s}. And since
\texttt{s\ \textless{}\textasciitilde{}\textgreater{}\ Either\ a\ q}, it
promotes a relationship on a ``potential branch'' to be a relationship on a
``whole''. A prism!
