...
a name, $\hat{T}$. If the masses are all positive and $\hat{J}_f$ is full-rank,
then $\hat{T}$ is a symmetric, positive-definite, invertible matrix (by
...
momenta, then: $\mathbf{p} = \hat{T} \dot{\mathbf{q}}$
...
$$
\mathcal{H}(\mathbf{q},\dot{\mathbf{q}}) = \frac{1}{2} \dot{\mathbf{q}}^T \hat{T} \dot{\mathbf{q}} + PE(\mathbf{q})
$$

Oh, but oops, the Hamiltonian has to be a function of $\mathbf{p}$, not of
$\dot{\mathbf{q}}$. Let’s remember that
$\dot{\mathbf{q}} = \hat{T}^{-1} \mathbf{p}$ and find the final form of our
Hamiltonian (after a bit of simplification, remembering that the inverse of a
symmetric matrix is also symmetric):

$$
\mathcal{H}(\mathbf{q},\mathbf{p}) = \frac{1}{2} \mathbf{p}^T \hat{T}^{-1} \mathbf{p} + PE(\mathbf{q})
$$
