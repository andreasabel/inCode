...
a name, $\hat{K}$. If the masses are all positive and $\hat{J}_f$ is full-rank,
then $\hat{K}$ is a symmetric, positive-definite, invertible matrix (by
...
momenta ($\mathbf{p} = \hat{K} \dot{\mathbf{q}}$), and also for kinetic energy
