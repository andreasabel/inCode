...
(Originally posted by Justin Le [/] on February 9, 2015)
...
It’s basically *identical* and exactly the same :O The only difference
...
We will need to of course re-write our trusty \[`testAuto`\]\[testauto\]
...
So, `replicateGets` uses \[`monoidAccum`\]\[macum\] (or, an `AutoM`
version) to accumulate a string. At every step, it adds `inpStr` to the
running accumulated string. `inpStr` is the result of repeating the the
string that `getLine` returns replicated `n` times — `n` being the
official “input” to the `AutoM` when we eventually run it.
...
`sealStateAuto` does exactly this. Give it an initial state, and the
...
`laggingSummer` is like `summer`, except all of the sums are delayed.
...
The `rec` keyword in proc/do blocks desugars to applications of a
...
In fact, for `f ~ Maybe`, this definition is identical to the one for
...
They are literally exactly the same…we just change the constructor to
...
[^4]: By the way, you might notice this pattern as something that seems
    more fit for `Reader` than `IO`. We’ll look at that later!

[^5]: Which really isn’t the point of these posts, anyway!

[^6]: Another exercise you can do if you wanted is to write the exact
    same instances, but for
    `newtype AutoOn a b = AutoOn (Auto a (Maybe b))` :)

[^7]: Admittedly, implicit short-circuiting with `Auto`s is actually
    often times a lot more of a headache than it’s worth; note that
    “switching” still works if you have a normal `Auto a (Maybe b)`;
    this is the approach that many libraries like
    [*auto*](https://github.com/mstksg/auto.) take — write switching
    combinators on normal `Auto a (Maybe b)`’s instead.
