...
`[uniformSample][uniformSample static]` for the *Static* module is at
the moment only in the yet unreleased *0.18* version of *hmatrix*. At
the time of this article’s writing, the current HEAD is at [commit
42a88fb](https://github.com/albertoruiz/hmatrix/tree/42a88fbcb6bd1d2c4dc18fae5e962bd34fb316a1).
I maintain my own [build of the
documentation](http://mstksg.github.io/hmatrix/) for reference.

You can use *stack* to make sure that you’re using the right version.
When you build these tests, put a proper *stack.yaml* in the directory,
including this modification to the `packages` field:
...
You can even add this to your global *stack.yaml* to make this
(completely reversible) change system-wide.

*stack* will then know (when you use *runghc* or *ghc* or *exec*) to use
the updated *hmatrix* version instead of the one on hackage.
