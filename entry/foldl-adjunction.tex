...
well as
\href{https://www.math3ma.com/blog/what-is-an-adjunction-part-1}{Tai-Dinae
...
adjunction is one way to formalize what this means.

In Haskell, with the \texttt{Adjunctions} typeclass (specifically,
\texttt{Functor} functors), this manifests as this: if
\texttt{F\ -\textbar{}\ U} (\texttt{F} is left adjoint to \texttt{U}, and
\texttt{U} is right adjoint to \texttt{F}), then all the ways of going ``out
of'' \texttt{F\ a} to \texttt{b} are the same as all the ways of going ``into''
\texttt{U\ b} from \texttt{a}. Ways of going out can be encoded as ways of going
...
adjunctions between \texttt{Functor} functors --- functors that the
\texttt{Functor} typeclass models (aka, endofunctors in Hask). For a more
general view of adjunctions in general, see the links above.
...
\texttt{EnvList\ r} is the left-adjoint of \texttt{Fold\ r}. A proof by hunch,
for now!
...
force}. Or, well\ldots I guess literally monads, since
...
adjunctions define a monad}, so what does our new knowledge of the
\texttt{Fold\ r} adjunction give us?
...
As it turns out, this is essentially the famous
...
\CommentTok{{-}{-} or, from Control.Monad.Representable.State}
...
As it turns out, this is equivalent to the
...
\CommentTok{{-}{-} or, from Control.Comonad.Representable.Store}
...
as a new way to compose computations dealing with \texttt{Fold} (via the
representable state monad and the representable store comonad).
