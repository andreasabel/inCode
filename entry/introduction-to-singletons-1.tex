...
\emph{\href{https://www.stackage.org/nightly-2017-11-27}{nightly-2017-11-27}}
...
to handle and reject it at runtime!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L18-36}
\OtherTok{closeDoor ::} \DataTypeTok{Door} \CharTok{'Opened -> Door '}\DataTypeTok{Closed}
...
\texttt{s\ \textasciitilde{}\ \textquotesingle{}Opened}\footnote{\texttt{\textasciitilde{}}
  ...
  be read as ``\texttt{s} is \texttt{\textquotesingle{}Opened}''.} -- that is,
\texttt{s} must be \texttt{\textquotesingle{}Opened}. That's because
...
that means that the \texttt{Door\ s} is \texttt{Door\ \textquotesingle{}Opened}.
We have an open door, so we can close-it-then-lock-it, using
...
\texttt{s\ \textasciitilde{}\ \textquotesingle{}Closed}, so our \texttt{Door\ s}
must be a \texttt{Door\ \textquotesingle{}Closed}. This allows us to just write
\texttt{SClosed\ -\textgreater{}\ lockDoor}.
...
\texttt{s\ \textasciitilde{}\ \textquotesingle{}Locked}, so our \texttt{Door\ s}
is a \texttt{Door\ \textquotesingle{}Locked}. Our door is ``already'' locked, so
we can just use
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L38-42}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L44-45}
\OtherTok{doorStatus ::} \DataTypeTok{SingDS}\NormalTok{ s }\OtherTok{->} \DataTypeTok{Door}\NormalTok{ s }\OtherTok{->} \DataTypeTok{DoorState}
...
\texttt{s\ \textasciitilde{}\ \textquotesingle{}Opened}. We explicitly wrote an
instance of \texttt{SingDSI} for \texttt{\textquotesingle{}Opened}, so GHC
\emph{knows} that there is a \texttt{SingDSI\ \textquotesingle{}Opened} instance
in existence, allowing you to use/create \texttt{x}. In the \texttt{SClosed}
branch, \texttt{s\ \textasciitilde{}\ \textquotesingle{}Closed}, so GHC knows
that there is a \texttt{SingDSI\ \textquotesingle{}Closed} instance (because we
wrote one explicitly!), and gives \emph{that} to you -- and so you are allowed
to use/create \texttt{x}. In the \texttt{SLocked} branch,
\texttt{s\ \textasciitilde{}\ \textquotesingle{}Locked}, and because we wrote a
\texttt{SingDSI\ \textquotesingle{}Locked} explicitly, we \emph{know} that a
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{SomeDoor} \FunctionTok{=} \DataTypeTok{UnsafeMkSomeDoor}
...
\KeywordTok{data} \DataTypeTok{SomeDoor}\OtherTok{ ::} \DataTypeTok{Type} \KeywordTok{where}
    \DataTypeTok{UnsafeMkSomeDoor} \OtherTok{::}
\NormalTok{      \{}\OtherTok{ someDoorState    ::} \DataTypeTok{DoorState}
\NormalTok{      ,}\OtherTok{ someDoorMaterial ::} \DataTypeTok{String}
...
the smart constructor/conversion function:
...
than going the other way around. One trick we often use is a CPS-style
existential type.
...
\texttt{SingDSI} incorrectly earlier, bu using the library guarantees that I
don't), and that they all work together smoothly.
...
for kind \texttt{Maybe\ k}, as well. And also for \texttt{{[}k{]}}, even! This
is a major advantage of using the \emph{singletons} library to manage your
