...
integrate it into your code today!\[^origin\] (Also, after [my previous April
Fools post](https://blog.jle.im/entry/verified-instances-in-haskell.html),
people have been asking me for an actual non-joke singletons post)
...
at runtime. Part 3 will go into the basics singleton's *defunctionalization*
system and how we can promote value-level functions to type-level functions, and
Part 4 will delve into deeper applications of defunctionalization.
...
All code is built on *GHC 8.2.2* and with the
*[lts-10.0](https://www.stackage.org/lts-10.0)* snapshot (so, singletons-2.3.1).
However, there are negligible changes in the GHC type system between GHC 8.0 and
8.2 (the only difference is in the libraries, more or less), so everything
should work on GHC 8.0 as well!
...
`haskell ghci> :t MkFoo :: Foo Int Foo Int ghci> :t MkFoo :: Foo Bool Foo Bool ghci> :t MkFoo :: Foo Either      -- requires -XPolyKinds where 'Foo' is defined Foo Either ghci> :t MkFoo :: Foo Monad       -- requires -XConstraintKinds Foo Monad`
...
2.  We're using the `DataKinds` extension to create both the *type* `DoorState`
    ...
    `haskell ghci> :k 'Opened DoorState ghci> :k 'Locked DoorState`

&nbsp;

1.  We're defining the `Door` type with a *phantom parameter* `s`. It's a
    phantom type because we don't actually have any *values* of type `s` in our
    data type\[^kind\] ...the `s` is only just there as a dummy parameter for
    the type.
    ...
    `haskell ghci> :t UnsafeMkDoor "Birch" :: Door 'Opened Door 'Opened ghci> :t UnsafeMkDoor "Iron" :: Door 'Locked Door 'Locked`

    We can also use the *TypeApplications* extension to write this in a bit more
    convenient way --

    `haskell ghci> :t UnsafeMkDoor @'Opened "Birch" Door 'Opened ghci> :t UnsafeMkDoor @'Locked "Iron" Door 'Locked`

Alternatively, we can define `Door` using [*GADT*
syntax](https://en.wikibooks.org/wiki/Haskell/GADT#Syntax) (which requires the
`GADTs` extension)\[^gadtnote\].

`haskell data Door :: DoorState -> Type where     UnsafeMkDoor :: { doorMaterial :: String } -> Door s`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L17-L18 closeDoor :: Door 'Opened -> Door 'Closed closeDoor (UnsafeMkDoor m) = UnsafeMkDoor m`

So, the `closeDoor` function will *only* take a `Door 'Opened` (an opened door).
And it will return a `Door 'Closed` (a closed door).

`haskell ghci> let myDoor = UnsafeMkDoor @'Opened "Spruce" ghci> :t myDoor Door 'Opened ghci> :t closeDoor myDoor Door 'Closed ghci> let yourDoor = UnsafeMkDoor @'Closed "Acacia" ghci> :t closeDoor yourDoor TYPE ERROR!  TYPE ERROR!`
...
`haskell ghci> :t closeDoor . closeDoor TYPE ERROR!  TYPE ERROR!  TYPE ERROR!`
...
lockDoor :: Door 'Closed -&gt; Door 'Locked lockDoor (UnsafeMkDoor m) =
UnsafeMkDoor m

openDoor :: Door 'Closed -&gt; Door 'Opened openDoor (UnsafeMkDoor m) =
(UnsafeMkDoor m) \`\`\`

`haskell ghci> :t closeDoor . openDoor Door 'Closed -> Door 'Closed ghci> :t lockDoor . closeDoor . openDoor Door 'Closed -> Door 'Locked ghci> :t lockDoor . openDoor TYPE ERROR!  TYPE ERROR!  TYPE ERROR!`

Because of the type of `lockDoor`, you *cannot* lock an opened door! Don't even
try! You'd have to close it first.

`haskell ghci> let myDoor = UnsafeMkDoor @'Opened "Spruce" ghci> :t myDoor Door 'Opened ghci> :t lockDoor Door 'Closed -> Door 'Closed ghci> :t lockDoor myDoor TYPE ERROR!  TYPE ERROR!  TYPE ERROR! ghci> :t closeDoor myDoor Door 'Closed ghci> :t lockDoor (closeDoor myDoor) Door 'Locked`
...
`haskell doorStatus :: Door s -> DoorState doorStatus _ = -- ?`
...
`haskell mkDoor :: DoorState -> String -> Door s mkDoor Opened = -- ? mkDoor Closed = -- ? mkDoor Locked = -- ?`

Ah hah, you say. That's easy!

`haskell mkDoor :: DoorState -> String -> Door s mkDoor Opened = UnsafeMkDoor mkDoor Closed = UnsafeMkDoor mkDoor Locked = UnsafeMkDoor`
...
`haskell ghci> :t mkDoor Opened "Acacia" :: Door 'Closed Door 'Closed`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L26-L29 data SingDS :: DoorState -> Type where     SOpened :: SingDS 'Opened     SClosed :: SingDS 'Closed     SLocked :: SingDS 'Locked`
...
`haskell SOpened :: SingDS 'Opened SClosed :: SingDS 'Closed SLocked :: SingDS 'Locked`
...
lockAnyDoor :: SingDS s -&gt; Door s -&gt; Door 'Locked lockAnyDoor sng door =
case sng of SOpened -&gt; lockDoor (closeDoor door) -- in this branch, s is
'Opened SClosed -&gt; lockDoor door -- in this branch, s is 'Closed SLocked
-&gt; door -- in this branch, s is 'Locked \`\`\`
...
-   If `SingDS s`'s pattern match goes down the `SOpened ->` case, then we
    *know* that `s ~ 'Opened`\[^eq\]. We know that `s` must be `'Opened`,
    because `SOpened :: SingDS 'Opened`, so there really isn't anything else the
    `s` in `SingDS s` could be!
    ...
    &lt;!-- Note that `lockDoor . closeDoor` will *only* compile if given a
    `Door --> <!-- 'Opened`, but because of our dependent pattern match, we
    *know* we have a --&gt; &lt;!-- `Door 'Opened`. --&gt;
...
-   Same for the `SClosed ->` branch -- since `SClosed :: SingDS 'Closed`, then
    ...
    &lt;!-- Again, `lockDoor :: Door 'Closed -> Door 'Locked`, so it would only
    work if --&gt; &lt;!-- given a `Door 'Closed` -- which we know we have,
...
-   For the `SLocked ->` branch, `SLocked :: SingDS 'Locked`, so `s ~ 'Locked`,
    ...
    &lt;!-- Note that `id :: Door 'Locked -> Door 'Locked` would not work for
    any other --&gt; &lt;!-- branch, and would be a compile-time error. `id`
    only works if you know your --&gt; &lt;!-- input is already
    `Door 'Locked`...which we know because of the dependent --&gt; &lt;!--
    pattern match. --&gt;
...
`haskell lockAnyDoor :: SingDS s -> (Door s -> Door 'Locked) lockAnyDoor = \case     SOpened -> lockDoor . closeDoor  -- in this branch, s is 'Opened     SClosed -> lockDoor              -- in this branch, s is 'Closed     SLocked -> id                    -- in this branch, s is 'Locked`
...
`haskell doorStatus :: SingDS s -> Door s -> DoorState doorStatus SOpened _ = Opened doorStatus SClosed _ = Closed doorStatus SLocked _ = Locked`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L37-L40 fromSingDS :: SingDS s -> DoorState fromSingDS SOpened = Opened fromSingDS SClosed = Closed fromSingDS SLocked = Locked`

Which we can use to write a nicer `doorStatus`

`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L42-L43 doorStatus :: SingDS s -> Door s -> DoorState doorStatus s _ = fromSingDS s`
...
lockAnyDoor\_ :: SingDSI s =&gt; Door s -&gt; Door 'Locked lockAnyDoor\_ =
lockAnyDoor singDS

doorStatus\_ :: SingDSI s =&gt; Door s -&gt; DoorState doorStatus\_ = doorStatus
singDS \`\`\`
...
`haskell ghci> let myDoor = UnsafeMkDoor @'Opened "Birch" ghci> :t lockAnyDoor SOpened myDoor -- our original method! Door 'Locked ghci> :t lockAnyDoor singDS myDoor  -- the power of type inference! Door 'Locked ghci> :t lockAnyDoor_ myDoor        -- no explicit singleton being passed! Door 'Locked`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L61-L65 withSingDSI :: SingDS s -> (SingDSI s => r) -> r withSingDSI sng x = case sng of     SOpened -> x     SClosed -> x     SLocked -> x`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L67-L68 lockAnyDoor__ :: SingDS s -> Door s -> Door 'Locked lockAnyDoor__ s d = withSingDSI s (lockAnyDoor_ d)`
...
`haskell withSingDSI :: SingDS s -> (SingDSI s => r) -> r flip  ($)   ::        a -> (        a -> r) -> r`
...
`haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door.hs#L70-L71 mkDoor :: SingDS s -> String -> Door s mkDoor _ = UnsafeMkDoor`
...
`haskell ghci> :t mkDoor SOpened "Oak" Door 'Opened ghci> :t mkDoor SLocked "Spruce" Door 'Locked`
...
\`\`\`haskell data DoorState = Opened | Closed | Locked deriving (Show, Eq)
...
`haskell -- not the actual code, but essentially what happens data Sing :: DoorState -> Type where     SOpened :: Sing 'Opened     SClosed :: Sing 'Closed     SLocked :: Sing 'Locked`
...
`haskell instance SingI 'Opened where     sing = SOpened instance SingI 'Closed where     sing = SClosed instance SingI 'Locked where     sing = SLocked`
...
`haskell ghci> sing :: Sing 'True STrue ghci> sing :: Sing ('Just 'Opened) SJust SOpened`

We also have `withSingI`, which is equivalent to our `withSingDSI` function
earlier.

`haskell withSingI :: Sing s -> (forall r. SingI s => r) -> r`
...
`` haskell ghci> :t SOpened `SCons` SClosed `SCons` SLocked `SCons` SNil Sing '[ 'Opened, 'Closed, 'Locked ] -- 'SCons is the singleton for `:` (cons), -- and 'SNil is the singleton for `[]` (nil) ``
...
3.  The singletons for `'Opened`, `'Closed`, and `'Locked`:
    ...
    `haskell SOpened :: Sing 'Opened SClosed :: Sing 'Closed SLocked :: Sing 'Locked`
...
`haskell ghci> fromSing SOpened Opened`
...
1.  Write a function to unlock a door, but only if the user enters an odd number
    ...
    `haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/DoorSingletons.hs#L60-L60 unlockDoor :: Int -> Door 'Locked -> Maybe (Door 'Closed)`
...
2.  Write a function that can open any door, taking a password, in "implicit
    ...
    `haskell -- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/DoorSingletons.hs#L65-L65 openAnyDoor :: SingI s => Int -> Door s -> Maybe (Door 'Opened)`
