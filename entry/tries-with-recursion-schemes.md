...
like `Trie k v = Fix (TrieF k v)`), or we can think of `TrieF` as a
"non-recursive view" into `Trie`. It's a way of *working* with `Trie a` *as if*
it were a non-recursive data type. Specifically, in our case, it's a
non-recursive view of a "single layer" of a `Trie`.
