...
\href{http://i.imgur.com/Vaaa2EC.gifv}{\includegraphics{http://i.imgur.com/Vaaa2EC.gif}}
...
\CommentTok{-- some helper patterns to pattern match on sized vectors}
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/hamilton/DoublePendulum.hs#L27-35}
\NormalTok{pattern }\DataTypeTok{V2}\OtherTok{ ::} \NormalTok{a }\OtherTok{->} \NormalTok{a }\OtherTok{->} \DataTypeTok{V.Vector} \DecValTok{2} \NormalTok{a}
\NormalTok{pattern }\DataTypeTok{V2} \NormalTok{x y }\OtherTok{<-} \NormalTok{(V.toList}\OtherTok{->}\NormalTok{[x,y])}
  \KeywordTok{where}
    \DataTypeTok{V2} \NormalTok{x y }\FunctionTok{=} \NormalTok{fromJust (V.fromList [x,y])}

\NormalTok{pattern }\DataTypeTok{V4}\OtherTok{ ::} \NormalTok{a }\OtherTok{->} \NormalTok{a }\OtherTok{->} \NormalTok{a }\OtherTok{->} \NormalTok{a }\OtherTok{->} \DataTypeTok{V.Vector} \DecValTok{4} \NormalTok{a}
\NormalTok{pattern }\DataTypeTok{V4} \NormalTok{x y z a }\OtherTok{<-} \NormalTok{(V.toList}\OtherTok{->}\NormalTok{[x,y,z,a])}
  \KeywordTok{where}
    \DataTypeTok{V4} \NormalTok{x y z a }\FunctionTok{=} \NormalTok{fromJust (V.fromList [x,y,z,a])}
\end{Highlighting}
\end{Shaded}
...
Where
\texttt{underlingPos\ ::\ System\ m\ n\ -\textgreater{}\ Phase\ n\ -\textgreater{}\ R\ n}.
...
\OtherTok{        ::} \DataTypeTok{Fractional} \NormalTok{a}
        \OtherTok{=>} \DataTypeTok{V.Vector} \DecValTok{2} \NormalTok{a}
        \OtherTok{->} \NormalTok{a}
    \NormalTok{potential (}\DataTypeTok{V2} \NormalTok{r _) }\FunctionTok{=} \FunctionTok{-} \DecValTok{10} \FunctionTok{/} \NormalTok{r}
...
\href{http://i.imgur.com/TDEHTcb.gifv}{\includegraphics{http://i.imgur.com/TDEHTcb.gif}}
...
launcher} included in the library, including:
