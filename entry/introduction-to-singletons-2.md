...
I mention this here before moving on to remind you of the “big picture” of *why*
singletons is important in the current state of Haskell. We really *want* to be
able to pattern match and branch on types, and we really *want* to be able to
encode existential types using the quantified type directly. However, “until
then”, we have `Sing s` instead of `s`, and
`forall s. SomeDoor (Sing s) (Door s)`, instead of `s ** Door s`.

You can see, syntactically, the appeal. The two are sort of designed to look
very syntactically similar. For now, I hope when you read and write the Haskell
