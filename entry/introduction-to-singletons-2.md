...
Essentially we’re converting a `SomeDoor` into a `Sing s` and `Door s`. We just
can’t know which `s`, a priori. So, we must handle it with a
`forall s. Sing s -> Door s -> r` (using *RankNTypes*) – a function that can
handle *any* `s` that it could possibly be.
