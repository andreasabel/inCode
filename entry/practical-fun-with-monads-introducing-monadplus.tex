...
\NormalTok{halve n }\OtherTok{=} \KeywordTok{do}                \CommentTok{{-}{-} <halve 8>   <halve 7>}
...
\NormalTok{    guard }\OperatorTok{$}\NormalTok{ newHealth }\OperatorTok{>} \DecValTok{0}           \CommentTok{{-}{-} fail/die immediately unless newHealth}
                                    \CommentTok{{-}{-}     is positive}
    \FunctionTok{return}\NormalTok{ newHealth                }\CommentTok{{-}{-} succeed with newHealth if not already}
                                    \CommentTok{{-}{-}     dead}

\CommentTok{{-}{-} an alternative but identical definition of \textasciigrave{}hit\textasciigrave{}, using >>= and >>}
\OtherTok{hit\textquotesingle{} ::} \DataTypeTok{Int} \OtherTok{{-}>} \DataTypeTok{Maybe} \DataTypeTok{Int}
\NormalTok{hit\textquotesingle{} currHealth }\OtherTok{=}\NormalTok{ guard (newHealth }\OperatorTok{>} \DecValTok{0}\NormalTok{) }\OperatorTok{>>} \FunctionTok{return}\NormalTok{ newHealth}
    \KeywordTok{where}
