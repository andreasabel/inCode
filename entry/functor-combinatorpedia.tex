...
way, this post also serves as a run-down of all the popular free structures in
Haskell :)
...
This is represented by the associated type \texttt{I\ t} (examples will be
provided below).
...
which contains ``at least one \texttt{f}'' (examples provided below).

They all support functions for ``converting'' back and forth, like
\texttt{toMF\ ::\ t\ f\ f\ \textasciitilde{}\textgreater{}\ MF\ f} (provided by
\emph{functor-combinators} in the \texttt{Monoidal} typeclass).
