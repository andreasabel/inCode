...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{Ap}\OtherTok{ ::}\NormalTok{ (}\DataTypeTok{Type} \OtherTok{{-}>} \DataTypeTok{Type}\NormalTok{) }\OtherTok{{-}>} \DataTypeTok{Type} \OtherTok{{-}>} \DataTypeTok{Type} \KeywordTok{where}
    \DataTypeTok{Pure}\OtherTok{ ::}\NormalTok{ a   }\OtherTok{{-}>} \DataTypeTok{Ap}\NormalTok{ f a}
    \DataTypeTok{Ap}\OtherTok{   ::}\NormalTok{ f a }\OtherTok{{-}>} \DataTypeTok{Ap}\NormalTok{ f (a }\OtherTok{{-}>}\NormalTok{ b) }\OtherTok{{-}>} \DataTypeTok{Ap}\NormalTok{ f b}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{instance} \DataTypeTok{Applicative}\NormalTok{ f }\OtherTok{=>} \DataTypeTok{Interpret} \DataTypeTok{Ap}\NormalTok{  f}
\KeywordTok{instance} \DataTypeTok{Apply}\NormalTok{       f }\OtherTok{=>} \DataTypeTok{Interpret} \DataTypeTok{Ap1}\NormalTok{ f}
...
\OtherTok{    ::} \DataTypeTok{Tensor}\NormalTok{ t i}
    \OtherTok{=>}\NormalTok{ t (}\DataTypeTok{Chain}\NormalTok{ t i f) (}\DataTypeTok{Chain}\NormalTok{ t i f) }\OperatorTok{\textasciitilde{}>} \DataTypeTok{Chain}\NormalTok{ t i f}

\NormalTok{appendChain1}
\OtherTok{    ::} \DataTypeTok{Associative}\NormalTok{ t}
    \OtherTok{=>}\NormalTok{ t (}\DataTypeTok{Chain1}\NormalTok{ t f) (}\DataTypeTok{Chain1}\NormalTok{ t f) }\OperatorTok{\textasciitilde{}>} \DataTypeTok{Chain1}\NormalTok{ t f}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{instance} \DataTypeTok{MonoidIn}\NormalTok{    t i f }\OtherTok{=>} \DataTypeTok{Interpret}\NormalTok{ (}\DataTypeTok{Chain}\NormalTok{  t i) f}
\KeywordTok{instance} \DataTypeTok{SemigroupIn}\NormalTok{ t   f }\OtherTok{=>} \DataTypeTok{Interpret}\NormalTok{ (}\DataTypeTok{Chain1}\NormalTok{ t  ) f}
...
\end{Shaded}
  ...
  \texttt{Div1}/\texttt{Div} are constructed in a way that is more well-suited
  for common usage patterns of \texttt{Divisible} methods.
...
\end{Shaded}
  ...
  \texttt{Decide} from \emph{{[}Data.Functor.Contravariant.Decide{]}{[}{]}} can
  be thought of as a deterministic sharding typeclass: You can combine two
  consumers along with a decision function on which consumer to use.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class} \DataTypeTok{Contravariant}\NormalTok{ f }\OtherTok{=>} \DataTypeTok{Decide}\NormalTok{ f }\KeywordTok{where}
\OtherTok{    decide ::}\NormalTok{ (a }\OtherTok{{-}>} \DataTypeTok{Either}\NormalTok{ b c) }\OtherTok{{-}>}\NormalTok{ f b }\OtherTok{{-}>}\NormalTok{ f c }\OtherTok{{-}>}\NormalTok{ f a}
                        \CommentTok{{-}{-} \^{} use the f b}
                          \CommentTok{{-}{-} \^{} use the f c}
\end{Highlighting}
\end{Shaded}

  \texttt{Conclude} from \emph{{[}Data.Functor.Contravariant.Conclude{]}{[}{]}},
  adds support for specifying an \texttt{f} that cannot be chosen by the
  decision function when used with \texttt{decide}.

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{class} \DataTypeTok{Decide}\NormalTok{ f }\OtherTok{=>} \DataTypeTok{Conclude}\NormalTok{ f }\KeywordTok{where}
\OtherTok{    conclude ::}\NormalTok{ (a }\OtherTok{{-}>} \DataTypeTok{Void}\NormalTok{) }\OtherTok{{-}>}\NormalTok{ f a}
\end{Highlighting}
\end{Shaded}
\item
  \textbf{List type}
...
\item
  ...
  \emph{Structurally}, \texttt{Div} is built like a linked list of
  \texttt{f\ x}s, which each link being existentially bound together:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{Div}\OtherTok{ ::}\NormalTok{ (}\DataTypeTok{Type} \OtherTok{{-}>} \DataTypeTok{Type}\NormalTok{) }\OtherTok{{-}>} \DataTypeTok{Type} \OtherTok{{-}>} \DataTypeTok{Type} \KeywordTok{where}
    \DataTypeTok{Conquer}\OtherTok{ ::} \DataTypeTok{Div}\NormalTok{ f a}
    \DataTypeTok{Divide}\OtherTok{  ::}\NormalTok{ f x }\OtherTok{{-}>} \DataTypeTok{Div}\NormalTok{ f y }\OtherTok{{-}>}\NormalTok{ (a }\OtherTok{{-}>}\NormalTok{ (x, y)) }\OtherTok{{-}>} \DataTypeTok{Div}\NormalTok{ f a}
\end{Highlighting}
\end{Shaded}
