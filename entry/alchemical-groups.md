Alchemical Groups: Advent of Code with Free Groups
==================================================
...
what it is describing is *exactly* the behavior of a [Free
Group](https://en.wikipedia.org/wiki/Free_group), from group theory!
...
`length . FG.toList`) to solve Part 1:
...
we can see that Part 2 is basically describing a *group homomorphism*. That is,
it talks about functions that map
...
seems like the problem might be deliberately written to obscure this choice from
us: it's implying that "cleaning, then reacting" is the only correct way, and
