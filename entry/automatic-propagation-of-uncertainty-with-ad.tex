...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\KeywordTok{./Uncertain.hs}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/uncertain/Uncertain.hs#L47-50}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/uncertain/Uncertain.hs#L55-71}
\NormalTok{liftUF}
...
\NormalTok{liftUF f us }\FunctionTok{=} \DataTypeTok{Un} \NormalTok{y vy}
  \KeywordTok{where}
    ...
    \NormalTok{y           }\FunctionTok{=} \NormalTok{fx }\FunctionTok{+} \NormalTok{partials }\FunctionTok{/} \DecValTok{2}
      \KeywordTok{where}
        \NormalTok{partials }\FunctionTok{=} \NormalTok{(vxs }\OtherTok{`dot`}\NormalTok{) }\FunctionTok{.} \NormalTok{diag}
                 \FunctionTok{$} \NormalTok{toList (fmap toList hess)}
    \NormalTok{vy          }\FunctionTok{=} \NormalTok{vxs }\OtherTok{`dot`} \NormalTok{toList ((}\FunctionTok{^}\DecValTok{2}\NormalTok{) }\FunctionTok{<$>} \NormalTok{dfxs)}
\end{Highlighting}
\end{Shaded}
...
\emph{\href{https://github.com/mstksg/uncertain}{uncertain}} project
(with \href{http://mstksg.github.io/uncertain}{documentation} that I'm
making, if you want to use it in your own projects, or see how I take
this and make it more robust for real-world applications. The project
also has more features on top of the basic things shown here. You can
install it using \emph{\href{http://haskellstack.org}{stack}} by adding
the github repository along with its latest commit hash to whatever
\texttt{stack.yaml} file you are using. In the future, I'll revise this
part to link to my actual package on Hackage!

\subsection{Verification and Accuracy}\label{verification-and-accuracy}

My \emph{\href{https://github.com/mstksg/uncertain}{uncertain}} package
...
ways? Stay tuned for the next part!\footnote{Or just look at my
  \href{https://github.com/mstksg/uncertain}{package} :)}
