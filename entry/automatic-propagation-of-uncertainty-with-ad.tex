...
And then a ``dot product'', which sums two \texttt{Foldable}s
component-wise and adds the results:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{dot ::} \NormalTok{(}\DataTypeTok{Foldable} \NormalTok{t, }\DataTypeTok{Num} \NormalTok{a) }\OtherTok{=>} \NormalTok{t a }\OtherTok{->} \NormalTok{t a }\OtherTok{->} \NormalTok{a}
\NormalTok{xs }\OtherTok{`dot`} \NormalTok{ys }\FunctionTok{=} \NormalTok{sum (zipWith (}\FunctionTok{*}\NormalTok{) (toList xs) (toList ys))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{liftUF f us }\FunctionTok{=} \DataTypeTok{Un} \NormalTok{y vy}
  \KeywordTok{where}
    ...
    \NormalTok{y           }\FunctionTok{=} \NormalTok{fx }\FunctionTok{+} \NormalTok{partials }\FunctionTok{/} \DecValTok{2}
      \KeywordTok{where}
        \NormalTok{partials }\FunctionTok{=} \NormalTok{dot vxsL }\FunctionTok{.} \NormalTok{diag}
...
you didn't miss any bugs. And at this point, we are done!
