...
Where \(f_x(x_0,y_0)\) is \(\frac{\partial f}{\partial x}\) at
\((x_0, y_0)\).
...
Where \(f_{xx}(\mu_X, \mu_Y)\) is
\(\frac{\partial^2 f}{{\partial x}^2}\) at \((\mu_X, \mu_Y)\)
...
\KeywordTok{instance} \DataTypeTok{Num} \NormalTok{a }\OtherTok{=>} \DataTypeTok{Num} \NormalTok{(}\DataTypeTok{Uncert} \NormalTok{a) }\KeywordTok{where}
    ...
    \DataTypeTok{Un} \NormalTok{x vx }\FunctionTok{-} \DataTypeTok{Un} \NormalTok{y vy }\FunctionTok{=} \DataTypeTok{Un} \NormalTok{(x }\FunctionTok{-} \NormalTok{y)    (vx }\FunctionTok{+} \NormalTok{vy)}
    \DataTypeTok{Un} \NormalTok{x vx }\FunctionTok{*} \DataTypeTok{Un} \NormalTok{y vy }\FunctionTok{=} \DataTypeTok{Un} \NormalTok{(x }\FunctionTok{*} \NormalTok{y)    (y}\FunctionTok{*}\NormalTok{y }\FunctionTok{*} \NormalTok{vx }\FunctionTok{+} \NormalTok{x}\FunctionTok{*}\NormalTok{x }\FunctionTok{*} \NormalTok{vy)}
...
But, wait\ldots{}this method is definitely not ideal. It's pretty
...
something we were supposed to? And\ldots{}if you look at the
...
And, now that we can automatically differentiate functions\ldots{}we can
use this knowledge directly in our implementations. Let's define a
universal ``lifter'' of single-variable functions.
...
derivatives and values of our function around the mean \texttt{x}.
