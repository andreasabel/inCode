...
\NormalTok{inject c}
    ...
    \FunctionTok{|}\NormalTok{ otherwise              }\FunctionTok{=}\NormalTok{ mempty       }\CommentTok{-- group identity element}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{day05b rawInput }\FunctionTok{=}\NormalTok{ minimum}
\NormalTok{    [ length }\FunctionTok{.}\NormalTok{ FG.toList }\FunctionTok{$}\NormalTok{ clean c polymer}
...
\NormalTok{    polymer }\FunctionTok{=}\NormalTok{ foldMap inject rawInput}
\end{Highlighting}
\end{Shaded}
...
\hypertarget{a-note-on-representation}{%
\subsection{A Note on Representation}\label{a-note-on-representation}}
