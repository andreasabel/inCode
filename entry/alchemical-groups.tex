...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Justin Le},
            pdftitle={Alchemical Groups: Advent of Code with Free Groups},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\title{Alchemical Groups: Advent of Code with Free Groups}
\author{Justin Le}
...
Now, I feel like this problem was deliberately written to obscure this fact, but
what it is describing is \emph{exactly} the behavior of a
...
\NormalTok{inject c}
    \FunctionTok{|}\NormalTok{ isAlpha c }\FunctionTok{&&}\NormalTok{ isLower c }\FunctionTok{=}\NormalTok{ returnFree c}
    \FunctionTok{|}\NormalTok{ isAlpha c }\FunctionTok{&&}\NormalTok{ isUpper c }\FunctionTok{=}\NormalTok{ invert }\FunctionTok{$}\NormalTok{ returnFree (toLower c)}
    \FunctionTok{|}\NormalTok{ otherwise              }\FunctionTok{=}\NormalTok{ mempty}
\end{Highlighting}
\end{Shaded}
...
\emph{imperative}, step-by-step process. Most naive implementations might scan
...
reduction \emph{might} matter (is it left to right, or right to left?).
...
we can see that Part 2 is basically describing a \emph{group homomorphism}. That
is, it talks about functions that map
...
\NormalTok{clean c }\FunctionTok{=}\NormalTok{ foldMapFree }\FunctionTok{$}\NormalTok{ \textbackslash{}d }\OtherTok{->}
        \KeywordTok{if}\NormalTok{ d }\FunctionTok{==}\NormalTok{ c}
          \KeywordTok{then}\NormalTok{ mempty}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{day05b (foldMap inject }\OtherTok{->}\NormalTok{ xs) }\FunctionTok{=}
    \FunctionTok{=}\NormalTok{ minimum [ length }\FunctionTok{$}\NormalTok{ FG.toList (clean c xs)}
              \FunctionTok{|}\NormalTok{ c }\OtherTok{<-}\NormalTok{ [}\CharTok{'a'} \FunctionTok{..} \CharTok{'z'}\NormalTok{]}
\NormalTok{              ]}
\end{Highlighting}
\end{Shaded}
...
it seems like the problem might be deliberately written to obscure this choice
