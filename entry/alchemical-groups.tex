...
library{[}\^{}pref{]}, which offers a free group type \texttt{FreeGroupL}, to
let us write:
...
\NormalTok{day05b (foldMap inject }\OtherTok{->}\NormalTok{ polymer) }\FunctionTok{=}
    \FunctionTok{=}\NormalTok{ minimum [ length }\FunctionTok{$}\NormalTok{ FG.toList (clean c polymer)}
