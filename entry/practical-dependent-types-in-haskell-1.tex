...
\texttt{dot\ ::\ KnownNat\ n\ =\textgreater{}\ R\ n\ -\textgreater{}\ R\ n}.
...
\texttt{(:\textless{}\#)}, you need a \texttt{KnownNat}). Then, when you
pattern match on the \texttt{(:\textless{}\#)} in \texttt{randomNet},
you ``release'' the \texttt{KnownNat\ n} that was stuffed in there by
\texttt{natList}.
...
typeclass instances that the constructor is hiding!)
...
requiring an \texttt{Integer} in it, which the pattern-matcher can then
take out.
...
Our implementation is also \emph{guaranteed} to have the right sized
matrices\ldots{}no worrying about using the right size parameters for
the right matrix in the right oder. GHC does it for you automatically!
