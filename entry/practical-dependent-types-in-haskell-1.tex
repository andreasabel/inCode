...
\KeywordTok{-} \FunctionTok{location:}
    \FunctionTok{git:} \NormalTok{git@github.com:mstksg/singletons.git}
    \FunctionTok{commit:} \NormalTok{v2.0.2}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L23-25}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L27-34}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L58-65}
...
different types, like for \texttt{Bool}:
...
we don't know about, we can pattern match on it. And in the branch that
\texttt{STrue} matches on, \texttt{b} is \texttt{\textquotesingle{}True}, and in
the branch that \texttt{SFalse} matches on, \texttt{b} is \texttt{False}.
...
However, for actual API's, it's usually much more convenient to \emph{not}
require the extra parameter, and have it be ``inferred'' in the way we've been
doing it before. So the \emph{singletons} library offers a typeclass we can use
to implicitly conjure up values of a singleton type -- \texttt{SingI}. We can
use \texttt{sing\ ::\ SingI\ s\ =\textgreater{}\ Sing\ s} to generate the
``inferred'' singleton:
...
instance. In order to create a \texttt{SNat\ ::\ Sing\ n}, you need a
\texttt{KnownNat\ n} instance in scope. GHC packs up the evidence that that
instance exists, and, when you pattern match on it, you also ``pattern match''
out that instance, too.

Another way you can think of it is that the \emph{creation} of
\texttt{SNat\ ::\ Sing\ n} requires the presence of \texttt{KnownNat\ n}. So if
you ever pattern match on a validly created \texttt{SNat}, the fact that that
\texttt{SNat} constructor even exists (instead of, say, being
\texttt{undefined}) is a \emph{witness} to that \texttt{KnownNat} instance, and
the type system lets us use this.

Now we have enough pieces of the puzzle:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L67-76}
...
\OtherTok{    go ::} \NormalTok{forall h hs'}\FunctionTok{.} \DataTypeTok{KnownNat} \NormalTok{h}
    ...
    \NormalTok{go hs }\FunctionTok{=} \KeywordTok{case} \NormalTok{hs }\KeywordTok{of}
...
The real heavy lifting is done by \texttt{go}, which takes the singleton
structure it needs and recursively calls it until it reaches the base case
(\texttt{SNil}, an output layer). We just call \texttt{go\ sing} to give it the
initial structure it needs. Note there, \texttt{sing\ ::\ Sing\ hs}, but this is
inferred, because \texttt{go} is
...
When possible, we like to write functions like \texttt{go}, which take explicit
singletons along with the types we care about. In a lot of situations, we'll
actually write our \emph{logic} using explicit singletons, and only use
\texttt{SingI} and implicit singletons at the external boundaries of our API
(like \texttt{randomNet}).
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L44-56}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L78-118}
...
\href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs\#L153-161}{\texttt{main}}
method is written identically to that of the other file and tests the identical
function.

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{$ }\KeywordTok{stack} \NormalTok{install hmatrix MonadRandom singletons}
