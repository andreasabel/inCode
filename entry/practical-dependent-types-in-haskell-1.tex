...
the input and output layers are called \emph{hidden} layers. Every node
``outputs'' a weighted sum of all of the outputs of the \emph{previous}
layer, plus an always-on ``bias'' term (so that its result can be
non-zero even when all of its inputs are zero). Symbolically, it looks
like:
...
\KeywordTok{data} \DataTypeTok{Weights} \FunctionTok{=} \DataTypeTok{W} \NormalTok{\{}\OtherTok{ wBiases ::} \FunctionTok{!}\NormalTok{(}\DataTypeTok{Vector} \DataTypeTok{Double}\NormalTok{)  }\CommentTok{-- m}
                 \NormalTok{,}\OtherTok{ wNodes  ::} \FunctionTok{!}\NormalTok{(}\DataTypeTok{Matrix} \DataTypeTok{Double}\NormalTok{)  }\CommentTok{-- m x n}
                 \NormalTok{\}                              }\CommentTok{-- "n to m" layer}
\end{Highlighting}
\end{Shaded}
...
which just lets us define \texttt{Network} by providing the type of its
\emph{constructors}, \texttt{O} and \texttt{(:\&\textasciitilde{})}. A
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkUntyped.hs#L57-85}
...
\NormalTok{$ }\KeywordTok{./NetworkUntyped.hs}
...
\tightlist
\item
  Almost every single function we wrote is partial. If we had passed in
  the incorrectly sized matrix/vector, or stored mismatched vectors in
...
that GHC offers. A \texttt{KnownNat\ n} constraint is more or less just
a way for you to ``get'' an Integer at runtime (with the \texttt{natVal}
function), so a \texttt{KnownNat\ n\ =\textgreater{}\ R\ n} is basically
a vector ``packaged'' with its size. Almost all operations in the
library require a \texttt{KnownNat} constraint.

Following this, a reasonable type for a \emph{network} might be
\texttt{Network\ 10\ 2}, taking 10 inputs and popping out 2 outputs.
This might be an ideal type to export, because it abstracts away the
size of the hidden layers. But it'd be nice for us to keep all of the
hidden layers in the type for now --- we'll see how it can be useful,
and we'll also talk about how to later hide/abstract it away when we
export the type.
...
inputs: a \texttt{Nat} (type level number, like \texttt{10} or
...
vectors to ``fit together'' Now if we ever pattern match on
\texttt{:\&\textasciitilde{}}, we know that the resulting matrices and
...
detour}\label{singletons-and-induction-detour}

The code for the updated \texttt{randomNet} takes a bit of explaining.

Let's say we want to construct a
\texttt{Network\ 4\ \textquotesingle{}{[}3,2{]}\ 1}. In true Haskell
fashion, we do this recursively (``inductively''). After all, we know
how to make a \texttt{Network\ i\ \textquotesingle{}{[}{]}\ o} (just
\texttt{O\ \textless{}\$\textgreater{}\ randomWieights}), and we know
how to create a \texttt{Network\ i\ (h\ \textquotesingle{}:\ hs)\ o} if
we had a \texttt{Network\ h\ hs\ o}. Now all we have to do is just
``pattern match'' on the type-level list, and\ldots{}
...
\texttt{hs} --- if it's empty, we'll get the \texttt{ØNL} constructor,
otherwise we'll get the \texttt{(:\textless{}\#)} constructor, etc:
...
(Note that we need \texttt{ScopedTypeVariables} and the
\texttt{forall\ ..\ hs\ ..} so that we can say \texttt{NatList\ hs} in
the function body.)

The reason why \texttt{NatList} and \texttt{:\textless{}\#} works for
this is that its constructors \emph{come with proofs} that the head is a
\texttt{KnownNat} and the tail is \texttt{KnownNats}. It's a part of the
GADT declaration. If you ever pattern match on \texttt{:\textless{}\#},
you get a \texttt{KnownNat\ n} constraint (that \texttt{randomWeights})
uses, and also a \texttt{KnownNats\ ns} constraint (that the recursive
call to \texttt{randomNet} uses).
...
structure (\texttt{NatList} here), and getting the singleton skeleton
from ``folding up'' using a typeclass (\texttt{KnownNats}, here).
\texttt{NatList\ hs} has exactly the structure we want, so we can fold
it down to a \texttt{Network\ i\ hs\ o}.
...
constraint. Where did it come from?
...
\texttt{(:\textless{}\#)}, you need a \texttt{KnownNat}). Then, in
\texttt{randomNet}, when you pattern match on the
\texttt{(:\textless{}\#)}, you ``release'' the \texttt{KnownNat\ n} that
was stuffed in there by \texttt{natList}.
...
is in scope/valid. But sometimes it helps to think of it in the way we
just did --- the instance is actually a ``thing'' that gets passed
around through typeclasses and GADT constructors/deconstructors. The
\texttt{KnownNat} instance gets put into \texttt{:\textless{}\#} by
\texttt{natList}, and is then taken out in the pattern match for
\texttt{randomWeights} to use.
...
\texttt{Integer\ -\textgreater{}\ ..}. \texttt{(:\textless{}\#)}
requiring a \texttt{KnownNat\ n\ =\textgreater{}} put into it is really
the same as requiring an \texttt{Integer} in it, which the
...
So now, you can use
\texttt{randomNet\ ::\ IO\ (Network\ 5\ \textquotesingle{}{[}4{]}\ 2)}
to get a random network of the desired dimensions! Much simpler than
before, right?
