...
it --- it's handled automatically by the magic of type inference! (take
\emph{that}, Idris!\footnote{Just kidding :)}) I consider this a very
big victory. One of the whole points of types is to give you less to
``worry about'', as a programmer. Here, we completely eliminate an
...
\texttt{Proxy}s), we can now ``pattern match'' on \texttt{hs} --- if
it's empty, we'll get the \texttt{ØNL} constructor, otherwise we'll get
the \texttt{(:\textless{}\#)} constructor, etc:
...
\texttt{KnownNat\ n} constraint for the \texttt{Proxy\ n} (that
...
value-level \emph{structure} from a type (often implemented with
typeclasses) and then inductively piggybacking on that structure's
constructors to build the thing you \emph{really} want (called
``elimination''). Here, we use \texttt{KnownNats\ hs} to build our
\texttt{NatList\ hs} structure, and use that structure to create our
\texttt{Network\ i\ hs\ o}.
