...
and to also walk through the ``why'' and high-level philosophy of the
way you structure your Haskell programs. It'll also hopefully instill an
intuition of a dependently typed work flow of ``exploring'' how
dependent types can help your current programs.
...
This process is called ``type-driven development'' by many --- start
...
de-facto idiom) because it gives us a way to ``get'' the information
from the \texttt{n} (an \texttt{Integer}) at runtime and use it in our
...
just convert the type to a ``dumb'' type as soon as I get the result).
All of these benefits come even when the caller doesn't \emph{care} at
all about dependently typed programming --- it's just a better style of
defining functions/offering an API!
...
\textbf{erased} at runtime.

We need to have a way to ``access'' the type (at run-time) as a
...
about the type parameter \texttt{ns}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\FunctionTok{>} \KeywordTok{case} \NormalTok{foo }\KeywordTok{of}   \CommentTok{-- foo :: NatList ns, but we don't know what `ns`}
        \NormalTok{Ø}\DataTypeTok{NL}         \OtherTok{->} \CommentTok{-- here, we know ns is `'[]`}
        \DataTypeTok{Proxy} \FunctionTok{:<#} \NormalTok{_ }\OtherTok{->} \CommentTok{-- here, we know ns is `m ': ms`}
\end{Highlighting}
\end{Shaded}
...
on. Now that we have an actual value-level \emph{structure} (the list of
