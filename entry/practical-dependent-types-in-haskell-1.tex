...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkUntyped.hs#L57-92}
...
\texttt{KnownNat} constraint, which basically allows the functions to
\emph{use} the information in the numeric parameter (the
\texttt{Integer} it represents, basically) at run-time. (More on this
later!)
...
it --- it's handled automatically by the magic of type inference! I
...
eliminate an \emph{entire dimension} of programmer concern.
...
\texttt{Network\ h\ hs\ o}. Now all we have to do is just ``pattern
...
\texttt{forall} so that can say \texttt{NatList\ hs} in the function
body.)

The reason why \texttt{NatList} and \texttt{:\textless{}\#} works for
this is that its constructors \emph{come with proofs} that the head's
type has a \texttt{KnownNat} and the tail's type has a
\texttt{KnownNats}. It's a part of the GADT declaration. If you ever
pattern match on \texttt{p\ :\textless{}\#\ ns}, you get a
\texttt{KnownNat\ n} constraint for the \texttt{p\ ::\ Proxy\ n} (that
\texttt{randomWeights}) uses, and also a \texttt{KnownNats\ ns}
constraint (that the recursive call to \texttt{randomNet} uses). We just
need the constraints, not the proxy/etc. themselves, so we can just
match on \texttt{\_\ :\textless{}\#\ \_}.

This is a common pattern in dependent Haskell: ``building up'' a
value-level \emph{structure} from a type (often done through typeclasses
like \texttt{KnownNats}) and then inductively piggybacking on that
...
that gets passed around through typeclasses and GADT
constructors/deconstructors. The \texttt{KnownNat} \emph{instance} gets
put \emph{into} \texttt{:\textless{}\#} by \texttt{natList}, and is then
taken \emph{out} in the pattern match so that \texttt{randomWeights} can
use it. (When we match on \texttt{\_\ :\textless{}\#\ \_}, we really are
saying that we don't care about the normal contents --- we just want the
typeclass instances that the constructor gives us!)
...
And also, our implementation is \emph{guarunteed} to have the right
sized matrices\ldots{}no worrying about using the right size parameters
for the right matrix in the right oder. GHC does it for you
automatically!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped.hs#L72-109}
...
But, with the types, writing the implementation became a \emph{joy}
again. And, you have the help of \emph{hole driven development}, too.
...
\def\labelenumi{\arabic{enumi}.}
\item
  ``I need an \texttt{R\ n}\ldots{}how do I get it?''

  ``Oh! I have an \texttt{L\ n\ m} and an \texttt{R\ m}, I can just
  multiply them!''
\item
  ``I need something of \emph{this} type\ldots{}how can I make it?''

  ``Ah, there's only one possible way to get this.'' Parametric
  polymorphism to the rescue.
\item
  ``I need to multiply this with \emph{something}\ldots{}but what?''

  ``Oh, if I just leave a \texttt{\_} hole there, GHC will actually tell
  me \emph{everything in scope} that can fit there!''
\end{enumerate}
