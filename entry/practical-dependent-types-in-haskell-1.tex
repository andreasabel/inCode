...
(\texttt{{[}randomVector{]}{[}{]}} and
\texttt{{[}uniformSample{]}{[}{]}} are from the
\emph{\href{http://hackage.haskell.org/package/hmatrix}{hmatrix}}
library, generating random vectors and matrices from a random
\texttt{Int} seed. We configure them to generate them with numbers
between -1 and 1)
...
\texttt{{[}randomVector{]}{[}randomVector\ static{]}} and
\texttt{{[}uniformSample{]}{[}uniformSample\ static{]}} for the
\emph{Static} module is at the moment only in the yet unreleased
\emph{0.18} version of \emph{hmatrix}. At the time of this article's
writing, the current HEAD is at
\href{https://github.com/albertoruiz/hmatrix/tree/42a88fbcb6bd1d2c4dc18fae5e962bd34fb316a1}{commit
42a88fb}. I maintain my own
\href{http://mstksg.github.io/hmatrix/}{build of the documentation} for
reference.

You can use \emph{stack} to make sure that you're using the right
version. When you build these tests, put a proper \emph{stack.yaml} in
the directory, including this modification to the \texttt{packages}
field:
...
You can even add this to your global \emph{stack.yaml} to make this
(completely reversible) change system-wide.

\emph{stack} will then know (when you use \emph{runghc} or \emph{ghc} or
\emph{exec}) to use the updated \emph{hmatrix} version instead of the
one on hackage.
