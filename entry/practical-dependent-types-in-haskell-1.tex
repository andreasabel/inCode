...
require a whole new set of tools. For example, our \texttt{Network}
types so far required you to specify their size in the program itself
...
What if we wanted to load a pre-trained network whose size we don't
know? How can we manipulate our networks in a dynamic and generic way?

What we're looking at here is a world where \emph{types} can depend on
dynamic values \ldots{} and values can depend on types. A world where
types become as much of a manipulatable citizen of our world as values
are.
