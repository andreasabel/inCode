...
effects of \texttt{f} and the effects of \texttt{x}, each exactly once. But
because we didn't really know what the ``effects'' of \texttt{IntConst} are, we
don't know exactly ``how'' to combine them\ldots{}but we can probably guess it
has to use both \texttt{Int}s. Let's try another one.
