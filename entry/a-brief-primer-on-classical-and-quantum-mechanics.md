...
Given a state of the world at time $t*0$, Newton's laws can be used to
"generate" the state of the world at time $t*0 + \\Delta t$.
...
Let's say I tell you an object's location at time $t*0$, and its location later
at time $t*1$, and the potential energy field. What path did that object take to
...
an equation that, for every abstract state at time $t*0$, gives you the abstract
state at time $t*0 + \\Delta t$.
...
the action of each path --- if Path $\\alpha$ has lower action than Path
...
point A at time $t*0$, and point B at time $t*1$. What path did the electron
take to get there?
...
+ \\Delta t$, for very large $\\Delta t$. From what we learned, this electron
