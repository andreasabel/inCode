...
implemented in a *parametrically polymorphic way*, meaning that it’s implemented
in a uniform way to handle *any* `i`, `hs`, or `o` the caller could possibly ask
for. (Provided that they satisfy the typeclass constraints, of course)
...
`o` you want or need (provided they satisfy the typeclass constraints). It takes
a `Network i hs o` for any `i`, `hs`, or `o`, and an `R i` for the same `i`, and
returns an `R o` for the same `o`. The way they are implemented does not depend
on the specific structure of the network.
...
And that’s sort of the secret to working with “types that depend on run-time” in
Haskell — you have to deal with them in a way that adapts to any type that can
come at you!
...
(I’ve specialized it there to work with `[Integer]` and `[Nat]`, but it works in
general with any singleton and the values it represents)
...
For example, if we had `foo :: Sing hs -> Int`, and we pass it to
`withSomeSing [1,2,3]`, it’ll *run* `foo`, setting with `hs` to be `'[1,2,3]`.
...
[singletons
section](https://blog.jle.im/entry/practical-dependent-types-in-haskell-1.html#singletons-and-induction)
for a review!) to make things more clear:

``` {.haskell}
-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L47-50
...
`someSing xs :: (forall hs. Sing hs -> r) -> r` is a function that’s waiting for
a `Sing hs -> r` that works for *any* `hs`. It’ll “run” the function with the
`hs` corresponding to `xs :: [Integer]`.
