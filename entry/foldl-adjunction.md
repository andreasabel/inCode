...
Conceptually this makes sense: to go out of `Either a a`, you have to handle the
situation of getting a `Left` and the situation of getting a `Right`. To go into
`(b, b)`, you have to able to ask what goes in the first field, and what goes in
the right field.
...
In the case of `Fold`, there is actually only one takes a `Fold r a` and returns
an `a`:
...
So to "seal the deal", let's find its pair, `tabulateAdjunction`. That means we
are looking for:
