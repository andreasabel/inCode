...
\hypersetup{breaklinks=true,
            bookmarks=true,
            pdfauthor={Justin Le},
            pdftitle={Rewriting my blog with Hakyll and Purescript},
            colorlinks=true,
            citecolor=blue,
            urlcolor=blue,
            linkcolor=magenta,
...
\title{Rewriting my blog with Hakyll and Purescript}
\author{Justin Le}
...
\href{https://github.com/faylang/fay/wiki}{fay}, because it was getting
a bit clunky to build/get working/integrate in the way that GHCJS
spoiled me to be accustomed to. In the future, I might return \ldots{}
...
power of Haskell (including all of \emph{base}'s concurrency
mechanisms), at the expense of large and unreadable javascript blobs.
Fay seems like just a weaker GHCJS to me\ldots{}it doesn't have all of
the awesome GHC things that make modern Haskell what it is (not just the
lack of base's identical API, but also \ldots{} no typeclasses? Lens?),
...
any actual Haskell code you'd write\ldots{} why even bother keeping up
...
refreshing\ldots{}not having to deal with fancy GADT's in Haskell was a
nice step back from the craziness that is type-level programming in
Haskell.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{function} \AttributeTok{__do}\NormalTok{() }\OperatorTok{\{}
...
came out to about the same filesize, about \emph{80 kB}. (Well, it is
about \emph{2 kB} of javascript, but it utilized all of \emph{jquery},
which implements a lot of the functionality.) Getting comparable
filesizes to jquery scripts is something that's pretty impressive to me!
...
\item
  Refactoring Haskell is a fun experience; would recommend to anyone to
  try it out at least once
\item
  purescript is an amazing and beautiful technology that I had the
