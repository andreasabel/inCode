...
\texttt{m} in every way\ldots{}except now, we have access to rudmentary
implementations of side-channels of \texttt{throwError} and \texttt{catchError}.

This is pretty useful\ldots{}to be able to add short-circuiting error behavior
to any Monad we wanted. But remember, \texttt{ExceptT} is not the ``point'' of
...
those database state things\ldots{}or those IORef state things\ldots{}or those
web query things\ldots{}or anything that cares to implement the interface!
...
\texttt{Reader}, \texttt{ST}, \texttt{STM}\ldots{}), except now we have access
to a rudimentary state getting-and-putting mechanism on a state of type
\texttt{s}, using a form of function composition. The implementation of the
\texttt{StateT} handles it under the hood.
...
IO\ldots{}anything that can embed IO can sequence my function/type''. The
generic ``embedding'' action is
