...
[tutorial](https://github.com/mstksg/auto/blob/master/tutorial/tutorial.md) or
the [docs](http://hackage.haskell.org/package/auto) might refresh your mind…feel
free to also leave a comment, stop by *\#haskell-auto* on freenode where I go by
*jle\`*, or [tweet me](https://twitter.com/mstk "Twitter")
...
values. So when we approach a chat bot, we have to think — what are the inputs,
...
for `Either InMessage UTCTime`…and drop in `Left im` whenever you get a message,
and `Right time` from a thread that just waits a minute and repeatedly throws in
times.
