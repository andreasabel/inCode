...
\(R\) as the \(N \times 2\) matrix of initial positions, and \(V\) as the
\(N \times 2\) matrix of initial velocities:
...
compute \(\mathrm{Tr} \left(M^T M \right)\) (since we guarantee that the mean of
\(M\) is 0), which is relatively easy peasy.
