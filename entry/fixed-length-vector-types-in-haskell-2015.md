...
`n` can really only be a certain “kind” of thing — a type that encodes a length.
We can represent this by giving it a “kind signature”:
...
messages. But we’re going to run with this for now…mostly for aesthetic reasons)
...
represents zero…something of type `S Z` represents 1…something of type `S (S Z)`
represents two.

Note that you can’t ever have anything like `S Bool`…that doesn’t work, because
`Bool` is of kind `*`, but `S` expects only `Nat`s.
...
Again,
`liftA2 (:#) :: Applicative f => f a -> f (Vec n a) -> f (Vec (S n) a)`…so this
only makes sense if `traverse f s` gives us a `Vec n a`. So we have to add that
as a constraint.
...
so…it’s actually a really bad idea for safety purposes unless you stick to only
using it with infinite lists or are very disciplined. (Unless you really want to
use list syntax, `fromListU` is probably a safer choice for finite lists!)
...
And this compiles fine…but gives the wrong behavior, or at least the behavior we
don’t want!
...
*DataKinds*…lifting a type and its value constructors to a kind and its types),
which is provided and defined for us by GHC in `GHC.TypeLits`.
...
type family `x + y`…because `GHC.TypeLits` already defines it for us! So, we can
instantly write….
...
Going through all of our other typeclasses/functions and making the adjustments…
(remembering that we can also derive `Functor`, `Traversable`, and `Foldable`
using GHC)
...
an article on its own…luckily, this article has already been written as [Part 1:
Dependent Types in
