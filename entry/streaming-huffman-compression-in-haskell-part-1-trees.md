...
have to encode any “stop” symbol — as soon as you reach a leaf of the tree, you
know that you have found a letter, and can move on.
...
for example) — and even if your implementation can — it might be fun to think
about how to implement `sizePQ` without it!)
...
This is a bit of an unrelated aside…but notice that we could have actually done
...
`evalState` is like partner of `execState` — it runs the state operation on the
given starting state, and outputs the final result (instead of the final state).
It takes an `s -> (a, s)`, an `s`, and applies the function to it and gives the
resulting `a` of the tuple.
