...
\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{SingKind}\NormalTok{ m, }\DataTypeTok{Semigroup}\NormalTok{ m)}
    ...
    \OtherTok{{-}>} \DataTypeTok{Demote}\NormalTok{ m}
    \OtherTok{{-}>} \DataTypeTok{Demote}\NormalTok{ m}
\NormalTok{x }\OperatorTok{<>}\NormalTok{ y }\OtherTok{=}\NormalTok{ withSomeSing x }\OperatorTok{$}\NormalTok{ \textbackslash{}sX }\OtherTok{{-}>}
\NormalTok{           withSomeSing y }\OperatorTok{$}\NormalTok{ \textbackslash{}sY }\OtherTok{{-}>}
\NormalTok{             fromSing (sX }\OperatorTok{\%<>}\NormalTok{ sY)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{instance} \DataTypeTok{Semigroup}\NormalTok{ (}\DataTypeTok{List}\NormalTok{ a) }\KeywordTok{where}
    \KeywordTok{type}\NormalTok{ xs }\OperatorTok{<>}\NormalTok{ ys }\OtherTok{=} \DataTypeTok{AppendList}\NormalTok{ xs ys}
\NormalTok{    (}\OperatorTok{\%<>}\NormalTok{) }\OtherTok{=}\NormalTok{ sAppendList}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{print} \OperatorTok{$}\NormalTok{ ((}\DecValTok{1}\OtherTok{::}\DataTypeTok{Integer}\NormalTok{) }\OtherTok{\textasciigrave{}Cons\textasciigrave{}} \DecValTok{2} \OtherTok{\textasciigrave{}Cons\textasciigrave{}} \DataTypeTok{Nil}\NormalTok{) }\OperatorTok{<>}\NormalTok{ (}\DecValTok{3} \OtherTok{\textasciigrave{}Cons\textasciigrave{}} \DecValTok{4} \OtherTok{\textasciigrave{}Cons\textasciigrave{}} \DataTypeTok{Nil}\NormalTok{)}
...
\KeywordTok{instance} \DataTypeTok{Semigroup} \DataTypeTok{N} \KeywordTok{where}
    \KeywordTok{type}\NormalTok{ xs }\OperatorTok{<>}\NormalTok{ ys }\OtherTok{=} \DataTypeTok{Plus}\NormalTok{ xs ys}
\NormalTok{    (}\OperatorTok{\%<>}\NormalTok{) }\OtherTok{=}\NormalTok{ sPlus}
...
\KeywordTok{instance} \DataTypeTok{Semigroup}\NormalTok{ a }\OtherTok{=>} \DataTypeTok{Semigroup}\NormalTok{ (}\DataTypeTok{Option}\NormalTok{ a) }\KeywordTok{where}
    \KeywordTok{type} \DataTypeTok{None}   \OperatorTok{<>}\NormalTok{ y      }\OtherTok{=}\NormalTok{ y}
    \KeywordTok{type}\NormalTok{ x      }\OperatorTok{<>} \DataTypeTok{None}   \OtherTok{=}\NormalTok{ x}
    \KeywordTok{type} \DataTypeTok{Some}\NormalTok{ x }\OperatorTok{<>} \DataTypeTok{Some}\NormalTok{ y }\OtherTok{=} \DataTypeTok{Some}\NormalTok{ (x }\OperatorTok{<>}\NormalTok{ y)}

    \DataTypeTok{SNone}   \OperatorTok{\%<>}\NormalTok{ y       }\OtherTok{=}\NormalTok{ y}
\NormalTok{    x       }\OperatorTok{\%<>} \DataTypeTok{SNone}   \OtherTok{=}\NormalTok{ x}
    \DataTypeTok{SSome}\NormalTok{ x }\OperatorTok{\%<>} \DataTypeTok{SSome}\NormalTok{ y }\OtherTok{=} \DataTypeTok{SSome}\NormalTok{ (x }\OperatorTok{\%<>}\NormalTok{ y)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{print} \OperatorTok{$} \DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S} \DataTypeTok{Z}\NormalTok{) }\OperatorTok{<>} \DataTypeTok{S} \DataTypeTok{Z}
\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S} \DataTypeTok{Z}\NormalTok{))}
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{print} \OperatorTok{$} \DataTypeTok{Some}\NormalTok{ (}\DataTypeTok{S} \DataTypeTok{Z}\NormalTok{) }\OperatorTok{<>} \DataTypeTok{Some}\NormalTok{ (}\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S} \DataTypeTok{Z}\NormalTok{)))}
...
\NormalTok{ghci}\OperatorTok{>} \FunctionTok{print} \OperatorTok{$} \DataTypeTok{None}       \OperatorTok{<>} \DataTypeTok{Some}\NormalTok{ (}\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S}\NormalTok{ (}\DataTypeTok{S} \DataTypeTok{Z}\NormalTok{)))}
...
\KeywordTok{class} \DataTypeTok{Functor}\NormalTok{ f }\KeywordTok{where}
    \KeywordTok{type} \DataTypeTok{Fmap}\NormalTok{ a b (}\OtherTok{g ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ b) (}\OtherTok{x ::}\NormalTok{ f a)}\OtherTok{ ::}\NormalTok{ f b}

\NormalTok{    sFmap}
\OtherTok{        ::} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{g            ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ b)}
...
\NormalTok{    fmapId}
\OtherTok{        ::} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{x ::}\NormalTok{ f a)}
        \OtherTok{{-}>} \DataTypeTok{Fmap}\NormalTok{ a a }\DataTypeTok{IdSym0}\NormalTok{ x }\OperatorTok{:\textasciitilde{}:}\NormalTok{ x}

    \CommentTok{{-}{-} | fmap f (fmap g x) = fmap (f . g) x}
\NormalTok{    fmapCompose}
\OtherTok{        ::} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{g ::}\NormalTok{ b }\OperatorTok{\textasciitilde{}>}\NormalTok{ c)}
        \OtherTok{{-}>} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{h ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ b)}
        \OtherTok{{-}>} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{x ::}\NormalTok{ f a   )}
        \OtherTok{{-}>} \DataTypeTok{Fmap}\NormalTok{ b c g (}\DataTypeTok{Fmap}\NormalTok{ a b h x) }\OperatorTok{:\textasciitilde{}:} \DataTypeTok{Fmap}\NormalTok{ a c (((}\OperatorTok{:.$}\NormalTok{) }\OperatorTok{@@}\NormalTok{ g) }\OperatorTok{@@}\NormalTok{ h) x}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{data} \DataTypeTok{ReturnSym0}\OtherTok{ ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ f a}
\KeywordTok{type} \KeywordTok{instance} \DataTypeTok{Apply}\NormalTok{ (}\DataTypeTok{ReturnSym0}\OtherTok{ ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ f a) (}\OtherTok{x ::}\NormalTok{ a) }\OtherTok{=} \DataTypeTok{Return}\NormalTok{ a x}

\KeywordTok{type} \DataTypeTok{KComp}\NormalTok{ a b c (}\OtherTok{g ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ f b) (}\OtherTok{h ::}\NormalTok{ b }\OperatorTok{\textasciitilde{}>}\NormalTok{ f c) (}\OtherTok{x ::}\NormalTok{ a) }\OtherTok{=} \DataTypeTok{Bind}\NormalTok{ b c (g }\OperatorTok{@@}\NormalTok{ x) h}
\KeywordTok{data} \DataTypeTok{KCompSym2}\NormalTok{ a b c g}\OtherTok{ h ::}\NormalTok{ (a }\OperatorTok{\textasciitilde{}>}\NormalTok{ f c)}
\KeywordTok{type} \KeywordTok{instance} \DataTypeTok{Apply}\NormalTok{ (}\DataTypeTok{KCompSym2}\NormalTok{ a b c g}\OtherTok{ h ::}\NormalTok{ a }\OperatorTok{\textasciitilde{}>}\NormalTok{ f c) (}\OtherTok{x ::}\NormalTok{ a) }\OtherTok{=} \DataTypeTok{KComp}\NormalTok{ a b c g h x}
...
\FunctionTok{return}\NormalTok{ x }\OtherTok{=}\NormalTok{ withSomeSing x }\OperatorTok{$}\NormalTok{ \textbackslash{}sX }\OtherTok{{-}>}
