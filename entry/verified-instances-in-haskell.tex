...
\section{Verify me, Captain}\label{verify-me-captain}
...
\OtherTok{        ::} \DataTypeTok{Sing} \NormalTok{(}\OtherTok{x ::} \NormalTok{a)}
        \OtherTok{->} \DataTypeTok{Sing} \NormalTok{y}
        \OtherTok{->} \DataTypeTok{Sing} \NormalTok{z}
...
\subsection{Automatic Singletons}\label{automatic-singletons}

Deriving \texttt{Sing} and \texttt{SingKind} and both versions of append is kind
of tedious, so it's useful to use template haskell to do it all for us:
...
\section{Naturally, Maybe}\label{naturally-maybe}
...
And everyone's favorite instance for \texttt{Maybe}, which lifts the underlying
semigroup:
