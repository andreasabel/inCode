...
Right: Use a list and \texttt{filter\ even} over a list from one to one
hundred.

Wrong: Use the list monad and \texttt{filter\ even} over a list from one
to one hundred.

Even more wrong but you couldn't really get more wrong in the first
place: Use the list monoid and \texttt{filter\ even} over a list from
one to one hundred.
...
\item
  ...
  \tightlist
  \item
    ``I have to print a string'': No; use the ``primitive''
    \texttt{putStrLn}.
  \item
    ``I have to turn an IO action returning an \texttt{Int} into an IO
    action returning a \texttt{Bool} (if it's even)'': No; use
    ...
    \texttt{IO\ Int}.
  \item
    ``I have to print a string twice, or multiple strings'': Kinda. You
    can use the monadic interface to do this using \emph{do} notation,
    or \texttt{mapM\_} or \texttt{(\textgreater{}\textgreater{})}. I say
    abuse because while using the monadic interface to do this is
    possible, it is a little overkill; \texttt{(*\textgreater{})} and
    \texttt{traverse\_} do the trick as well.
  \item
  ...
  \item
    ...
    \emph{pipes} or \emph{conduit}. (Working directly with) the
    \texttt{IO} type is actually notoriously bad at this. Don't use it;
    and of course, don't use its monad instance either.
  \item
    ``I have to directly use the result of one IO action in order to
    decide which IO action should happen next'': Yes, this is a use case
    for IO's monadic interface.
  \item
    ``What is one type can I use with a \emph{do} block'': Yes, IO
    Monad. Because only monads can be used with \emph{do} blocks.
  \item
    ``What is a Monad I can use as the underlying monad for my monad
    transformer?'': Yes, IO Monad. Because you need a monad in
    particular.
  \item
  ...
  \end{itemize}
\item
  \textbf{Edit}: So a lot of people seem to be getting the impression
  that I advocate not using the word ``monad'' to describe things like
  ``binding'' expressions, do blocks, using
  \texttt{(\textgreater{}\textgreater{}=)} with \texttt{IO}, etc. I am
  ...
  and only leads to more confusion, and I think it might get in the way
  of learning and making important connections.
