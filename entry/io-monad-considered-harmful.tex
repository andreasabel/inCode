...
\item
  ...
  \item
    ...
    to "map" your \texttt{Int\ -\textgreater{}\ Bool} onto the \texttt{IO\ Int}.
  \item
  ...
  \item
    "I have to do line-by-line constant-memory text processing of a large file":
    No; please use a streaming combinator library like \emph{pipes} or
    ...
    instance either.
  \item
    "I have to directly use the result of one IO action in order to decide which
    IO action should happen next": Yes, this is a use case for IO's monadic
    interface.
  \item
    "What is one type can I use with a \emph{do} block": Yes, IO Monad. Because
    only monads can be used with \emph{do} blocks.
  \item
