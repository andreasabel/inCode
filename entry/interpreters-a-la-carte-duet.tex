...
\NormalTok{parseOp inp }\FunctionTok{=} \KeywordTok{case} \FunctionTok{words}\NormalTok{ inp }\KeywordTok{of}
    \StringTok{"snd"}\FunctionTok{:}\NormalTok{c    }\FunctionTok{:}\NormalTok{_   }\OtherTok{->} \DataTypeTok{OSnd}\NormalTok{ (addr c)}
    \StringTok{"set"}\FunctionTok{:}\NormalTok{(x}\FunctionTok{:}\NormalTok{_)}\FunctionTok{:}\NormalTok{y}\FunctionTok{:}\NormalTok{_ }\OtherTok{->} \DataTypeTok{OBin}\NormalTok{ (}\FunctionTok{const} \FunctionTok{id}\NormalTok{) x (addr y)}
    ...
    \StringTok{"mod"}\FunctionTok{:}\NormalTok{(x}\FunctionTok{:}\NormalTok{_)}\FunctionTok{:}\NormalTok{y}\FunctionTok{:}\NormalTok{_ }\OtherTok{->} \DataTypeTok{OBin} \FunctionTok{mod}\NormalTok{        x (addr y)}
...
\NormalTok{    addr [c] }\FunctionTok{|} \FunctionTok{isAlpha}\NormalTok{ c }\FunctionTok{=} \DataTypeTok{Left}\NormalTok{ c}
\NormalTok{    addr str }\FunctionTok{=} \DataTypeTok{Right}\NormalTok{ (}\FunctionTok{read}\NormalTok{ str)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{parseProgram }\FunctionTok{=}\NormalTok{ fromJust }\FunctionTok{.}\NormalTok{ P.fromList }\FunctionTok{.} \FunctionTok{map}\NormalTok{ parseOp }\FunctionTok{.} \FunctionTok{lines}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::} \DataTypeTok{Monad}\NormalTok{ m                              }\CommentTok{-- m is the monad to interpret in}
    \OtherTok{=>}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.} \DataTypeTok{StateCommand}\NormalTok{ x }\OtherTok{->}\NormalTok{ m x)    }\CommentTok{-- a way to interpret each primitive in 'm'}
...
\NormalTok{stepProg }\FunctionTok{=}\NormalTok{ dPeek }\FunctionTok{>>=}\NormalTok{ \textbackslash{}}\KeywordTok{case}
...
\NormalTok{      dJump }\FunctionTok{=<<} \KeywordTok{if}\NormalTok{ xVal }\FunctionTok{>} \DecValTok{0}
        \KeywordTok{then}\NormalTok{ addrVal y}
        \KeywordTok{else} \FunctionTok{return} \DecValTok{1}
  \KeywordTok{where}
    \CommentTok{-- | Addr is `Either Char Int` -- `Left` means a register (so we use}
    \CommentTok{-- `dGet`) and `Right` means a direct integer value.}
\NormalTok{    addrVal (}\DataTypeTok{Left}\NormalTok{ r ) }\FunctionTok{=}\NormalTok{ dGet r}
\NormalTok{    addrVal (}\DataTypeTok{Right}\NormalTok{ x) }\FunctionTok{=} \FunctionTok{return}\NormalTok{ x}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{interpMem }\FunctionTok{=}\NormalTok{ \textbackslash{}}\KeywordTok{case}
    ...
    \DataTypeTok{MJump}\NormalTok{ n  }\OtherTok{->} \KeywordTok{do}
      \DataTypeTok{Just}\NormalTok{ t' }\OtherTok{<-}\NormalTok{ P.moveN n }\FunctionTok{<$>}\NormalTok{ use psOps}
\NormalTok{      psOps }\FunctionTok{.=}\NormalTok{ t'}
    \DataTypeTok{MPeek}    \OtherTok{->}\NormalTok{ use (psOps }\FunctionTok{.}\NormalTok{ P.focus)}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::} \DataTypeTok{Monad}\NormalTok{ m}
    \OtherTok{=>}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.}\NormalTok{ (}\DataTypeTok{Mem}\NormalTok{ x }\FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{ x) }\OtherTok{->}\NormalTok{ m x)}
...
\NormalTok{partA ops }\FunctionTok{=}\NormalTok{ getFirst}
          \FunctionTok{.} \FunctionTok{flip}\NormalTok{ A.evalAccum }\FunctionTok{mempty}
          \FunctionTok{.}\NormalTok{ execWriterT}
          \FunctionTok{.} \FunctionTok{flip}\NormalTok{ runStateT (}\DataTypeTok{PS}\NormalTok{ ops M.empty)}
...
\NormalTok{    guard }\FunctionTok{.} \FunctionTok{not} \FunctionTok{$} \FunctionTok{null}\NormalTok{ outA }\FunctionTok{&&} \FunctionTok{null}\NormalTok{ outB}
    \FunctionTok{return} \FunctionTok{$} \FunctionTok{length}\NormalTok{ outB}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{partB ops }\FunctionTok{=} \FunctionTok{maybe}\NormalTok{ (}\FunctionTok{error} \StringTok{"`many` cannot fail"}\NormalTok{) }\FunctionTok{sum}
          \FunctionTok{.} \FunctionTok{flip}\NormalTok{ evalState s0}
          \FunctionTok{.}\NormalTok{ runMaybeT}
...
\NormalTok{main }\FunctionTok{=} \KeywordTok{do}
    \FunctionTok{print} \FunctionTok{$}\NormalTok{ partA (parseProgram testProg)}
    \FunctionTok{print} \FunctionTok{$}\NormalTok{ partB (parseProgram testProg)}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.}\NormalTok{ f x }\OtherTok{->}\NormalTok{ g x }\OtherTok{->}\NormalTok{ r)}
...
\begin{Highlighting}[]
\NormalTok{bimap1}
\OtherTok{    ::}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.}\NormalTok{ f x }\OtherTok{->}\NormalTok{ h x)}
    \OtherTok{->}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.}\NormalTok{ g x }\OtherTok{->}\NormalTok{ j x)}
    \OtherTok{->}\NormalTok{ (f }\FunctionTok{:|:}\NormalTok{ g) a}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bimap1 (}\DataTypeTok{C} \DecValTok{0} \FunctionTok{:&:}\NormalTok{) }\FunctionTok{id}
\OtherTok{    ::}\NormalTok{ (      }\DataTypeTok{Mem}       \FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
    \OtherTok{->}\NormalTok{ ((}\DataTypeTok{C} \DataTypeTok{Int} \FunctionTok{:&:} \DataTypeTok{Mem}\NormalTok{) }\FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::}\NormalTok{ (}\KeywordTok{forall}\NormalTok{ x}\FunctionTok{.}\NormalTok{ f x }\OtherTok{->}\NormalTok{ g x)}
    \OtherTok{->} \DataTypeTok{Program}\NormalTok{ f a}
    \OtherTok{->} \DataTypeTok{Program}\NormalTok{ g a}

\NormalTok{interpretWithMonad (singleton }\FunctionTok{.}\NormalTok{ bimap1 (}\DataTypeTok{C} \DecValTok{0} \FunctionTok{:&:}\NormalTok{) }\FunctionTok{id}\NormalTok{)}
...
\NormalTok{handleFSum hs }\FunctionTok{=}\NormalTok{ ifoldMapFSum }\FunctionTok{$}\NormalTok{ \textbackslash{}i }\OtherTok{->}\NormalTok{ runHandle (}\FunctionTok{index}\NormalTok{ i hs)}
\end{Highlighting}
\end{Shaded}
