...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L110-L113}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L115-L125}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L131-L142}
...
\NormalTok{      when (x }\FunctionTok{/=} \DecValTok{0}\NormalTok{) }\FunctionTok{$} \KeywordTok{do}      \CommentTok{-- don't rcv if the register parameter is 0}
...
For now,
\href{https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs\#L127-L129}{I've
added \texttt{MonadAccum}} and
\href{https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs\#L260-L262}{appropriate
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L155-L161}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L163-L172}
...
\texttt{runPromptM}, by using \texttt{\textgreater{}\textbar{}\textless{}} to
generate our compound interpreters.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L144-L145}
...
valid \texttt{CRcv} is used \texttt{tell}'d, thanks to laziness. If we only ask
for the \texttt{First\ Int}, it'll stop running the rest of the computation!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L147-L153}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L174-L183}
...
\NormalTok{    outA }\OtherTok{<-}\NormalTok{ execWriterT }\FunctionTok{$}
\NormalTok{      zoom _1 }\FunctionTok{.}\NormalTok{ many }\FunctionTok{$}\NormalTok{ runPromptM (interpMem }\FunctionTok{>|<}\NormalTok{ interpComB) stepProg}
\NormalTok{    outB }\OtherTok{<-}\NormalTok{ execWriterT }\FunctionTok{$}
\NormalTok{      zoom _2 }\FunctionTok{.}\NormalTok{ many }\FunctionTok{$}\NormalTok{ runPromptM (interpMem }\FunctionTok{>|<}\NormalTok{ interpComB) stepProg}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L185-L193}

\OtherTok{partB ::} \DataTypeTok{P.PointedList} \DataTypeTok{Op} \OtherTok{->} \DataTypeTok{Int}
\NormalTok{partB ops }\FunctionTok{=}\NormalTok{ sum }\FunctionTok{.}\NormalTok{ concat}
...
\texttt{stepB}. To get our final answer, we only need to sum.
...
\href{https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs\#L200-L243}{my
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L195-L198}
...
\subsection{Pushing the Boundaries}\label{pushing-the-boundaries}}

The \emph{type-combinators} library opens up a lot of doors to combining modular
interpreters in more complex ways, as well!

\hypertarget{functor-conjunctions}{%
\subsubsection{Functor conjunctions}\label{functor-conjunctions}}
...
And hey, we can even make things more type-safe by attaching a
\texttt{Lens\textquotesingle{}}, instead:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{type} \DataTypeTok{PSL}\NormalTok{ s }\FunctionTok{=} \DataTypeTok{ALens'}\NormalTok{ s }\DataTypeTok{ProgState}

\CommentTok{-- | Peek into Thread 0}
\DataTypeTok{C}\NormalTok{ (_1 }\FunctionTok{.}\NormalTok{ tState) }\FunctionTok{:&:} \DataTypeTok{MPk}\OtherTok{         ::}\NormalTok{ (}\DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ (}\DataTypeTok{Thread}\NormalTok{, t)) }\FunctionTok{:&:} \DataTypeTok{Mem}\NormalTok{) }\DataTypeTok{Op}

\CommentTok{-- | Get register 'c' of Thread 1}
\DataTypeTok{C}\NormalTok{ (_2 }\FunctionTok{.}\NormalTok{ tState) }\FunctionTok{:&:} \DataTypeTok{MGet} \CharTok{'c'}\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ (t, }\DataTypeTok{Thread}\NormalTok{)) }\FunctionTok{:&:} \DataTypeTok{Mem}\NormalTok{) }\DataTypeTok{Int}
\end{Highlighting}
\end{Shaded}

Something of type \texttt{(C\ (PSL\ (Thread,\ t))\ :\&:\ Mem)\ Op} is an action
that gets an \texttt{Op}, tagged with a lens to access a \texttt{ProgState} in a
state of type \texttt{(Thread,\ t)}. So, instead of manually zooming ad-hoc, we
can zoom based on a lens attached to a \texttt{Mem}.
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/duet/Duet.hs#L247-L258}

\NormalTok{interpMemPSL}
\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{MonadState}\NormalTok{ s m, }\DataTypeTok{MonadFail}\NormalTok{ m)}
    \OtherTok{=>} \DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ s) a}
    \OtherTok{->} \DataTypeTok{Mem}\NormalTok{ a}
    \OtherTok{->}\NormalTok{ m a}
\NormalTok{interpMemPSL (}\DataTypeTok{C}\NormalTok{ (cloneLens}\OtherTok{->}\NormalTok{l)) }\FunctionTok{=}\NormalTok{ \textbackslash{}}\KeywordTok{case}
    \DataTypeTok{MGet}\NormalTok{ c   }\OtherTok{->}\NormalTok{ use (l }\FunctionTok{.}\NormalTok{ psRegs }\FunctionTok{.}\NormalTok{ at c }\FunctionTok{.}\NormalTok{ non }\DecValTok{0}\NormalTok{)}
    \DataTypeTok{MSet}\NormalTok{ c x }\OtherTok{->}\NormalTok{ l }\FunctionTok{.}\NormalTok{ psRegs }\FunctionTok{.}\NormalTok{ at c }\FunctionTok{.}\NormalTok{ non }\DecValTok{0} \FunctionTok{.=}\NormalTok{ x}
    \DataTypeTok{MJmp}\NormalTok{ n   }\OtherTok{->} \KeywordTok{do}
      \DataTypeTok{Just}\NormalTok{ t' }\OtherTok{<-}\NormalTok{ P.moveN n }\FunctionTok{<$>}\NormalTok{ use (l }\FunctionTok{.}\NormalTok{ psTape)}
\NormalTok{      l }\FunctionTok{.}\NormalTok{ psTape }\FunctionTok{.=}\NormalTok{ t'}
    \DataTypeTok{MPk}      \OtherTok{->}\NormalTok{ use (l }\FunctionTok{.}\NormalTok{ psTape }\FunctionTok{.}\NormalTok{ P.focus)}
\end{Highlighting}
\end{Shaded}

Note that since \texttt{PSL} is an \texttt{ALens\textquotesingle{}} (which is
required, if we want to store a lens in a newtype), we have to use
\texttt{cloneLens\ ::\ ALens\textquotesingle{}\ s\ a\ -\textgreater{}\ Lens\ s\ a}
to convert it back to a normal \texttt{Lens}.

We can then use the analogy of \texttt{\textgreater{}\textbar{}\textless{}},
\texttt{uncurryFan}:
...
\NormalTok{uncurryFan interpMemPSL}
\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{MonadState}\NormalTok{ s m, }\DataTypeTok{MonadFail}\NormalTok{ m)}
    \OtherTok{=>}\NormalTok{ (}\DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ s) }\FunctionTok{:&:} \DataTypeTok{Mem}\NormalTok{) a}
    \OtherTok{->}\NormalTok{ m a}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{uncurryFan interpMemPSL }\FunctionTok{>|<}\NormalTok{ interpComB}
\OtherTok{    ::}\NormalTok{ ( }\DataTypeTok{MonadWriter}\NormalTok{ [}\DataTypeTok{Int}\NormalTok{] m}
\NormalTok{       , }\DataTypeTok{MonadFail}\NormalTok{ m}
\NormalTok{       , }\DataTypeTok{MonadState}\NormalTok{ s m}
\NormalTok{       )}
    \OtherTok{=>}\NormalTok{ ((}\DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ s) }\FunctionTok{:&:} \DataTypeTok{Mem}\NormalTok{) }\FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
    \OtherTok{->}\NormalTok{ m a}
\end{Highlighting}
\end{Shaded}

\hypertarget{manipulating-disjunctions-and-conjunctions}{%
\subsubsection{Manipulating Disjunctions and
Conjunctions}\label{manipulating-disjunctions-and-conjunctions}}

So, we have a \texttt{Mem\ :\textbar{}:\ Conj}. How could we ``tag'' our
\texttt{Mem} after-the-fact, to add \texttt{C\ (PSL\ s)}? We can manipulate the
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{type} \DataTypeTok{MemAt}\NormalTok{ s }\FunctionTok{=} \DataTypeTok{C}\NormalTok{ (}\DataTypeTok{PSL}\NormalTok{ s) }\FunctionTok{:&:} \DataTypeTok{Mem}

\NormalTok{bimap1 (}\DataTypeTok{C}\NormalTok{ (_1 }\FunctionTok{.}\NormalTok{ tState) }\FunctionTok{:&:}\NormalTok{) id}
\OtherTok{    ::}\NormalTok{ (       }\DataTypeTok{Mem}       \FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
    \OtherTok{->}\NormalTok{ (}\DataTypeTok{MemAt}\NormalTok{ (}\DataTypeTok{Tread}\NormalTok{, t) }\FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
\end{Highlighting}
\end{Shaded}

Which we can use to re-tag a \texttt{Prompt\ (Mem\ :\textbar{}:\ Com)}, with the
help of \texttt{runPromptM}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{\textbackslash{}f }\OtherTok{->}\NormalTok{ runPromptM (prompt }\FunctionTok{.}\NormalTok{ f)}
\OtherTok{    ::}\NormalTok{ (forall x}\FunctionTok{.}\NormalTok{ f x }\OtherTok{->}\NormalTok{ g x)}
    \OtherTok{->} \DataTypeTok{Prompt}\NormalTok{ f a}
    \OtherTok{->} \DataTypeTok{Prompt}\NormalTok{ g a}

\NormalTok{runPromptM (prompt }\FunctionTok{.}\NormalTok{ bimap1 ((}\DataTypeTok{C}\NormalTok{ _1 }\FunctionTok{.}\NormalTok{ tState) }\FunctionTok{:&:}\NormalTok{) id)}
\OtherTok{    ::} \DataTypeTok{Prompt}\NormalTok{ (      }\DataTypeTok{Mem}        \FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
    \OtherTok{->} \DataTypeTok{Prompt}\NormalTok{ (}\DataTypeTok{MemAt}\NormalTok{ (}\DataTypeTok{Tread}\NormalTok{, t) }\FunctionTok{:|:} \DataTypeTok{Com}\NormalTok{) a}
\end{Highlighting}
\end{Shaded}

\hypertarget{many-sets-of-primitives}{%
\subsection{Many sets of primitives}\label{many-sets-of-primitives}}

Instead of
\texttt{f\ \textgreater{}\textbar{}\textless{}\ g\ \textgreater{}\textbar{}\textless{}\ h},
you can use \texttt{FSum\ \textquotesingle{}{[}f,\ g,\ h{]}} to combine multiple
...
\NormalTok{prompt (finj (}\DataTypeTok{MGet} \CharTok{'c'}\NormalTok{))}\OtherTok{ ::} \DataTypeTok{Prompt}\NormalTok{ (}\DataTypeTok{FSum} \CharTok{'[Mem, Com, Foo]) Int}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{handleFSum hs }\FunctionTok{=}\NormalTok{ ifoldMapFSum }\FunctionTok{$}\NormalTok{ \textbackslash{}i }\OtherTok{->}\NormalTok{ runHandle }\FunctionTok{$}\NormalTok{ index i hs}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{runPromptM (handleFSum ( }\DataTypeTok{Handle}\NormalTok{ handleMem}
...
\NormalTok{           )}
\OtherTok{    ::} \DataTypeTok{Prompt}\NormalTok{ (}\DataTypeTok{FSum} \CharTok{'[Mem, Com, Foo]) a -> m a}
\end{Highlighting}
\end{Shaded}

\hypertarget{endless-possibilities}{%
\subsubsection{Endless Possibilities}\label{endless-possibilities}}
