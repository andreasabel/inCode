...
\protect\hyperlink{streams}{from github} and tried interactively online
{[}at FPComplete{]}{[}streamsint{]})

!!\includegraphics{\#streams}:machines/Stream.hs {[}streamsint{]}:
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"data List"}\DecValTok{1} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"myList ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"myStream ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"newtype Stream"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"streamToList ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"testStream ::"} \StringTok{"testStream_ ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"streamFrom ::"}\DecValTok{2} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Stream.hs }\StringTok{"charStream ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\texttt{Stream\ Char}\ldots{}\texttt{Char} is the type of
output/elements in the stream, the ``head'' when we pattern match. But
where is the \texttt{Int} that is the state of our stream in the type
\texttt{Stream\ Char}\ldots{}?

Can we even write a function
\texttt{getState\ ::\ Stream\ b\ -\textgreater{}\ s} that works in
general for all streams?

Hm. If the state of our stream can have a type totally unrelated to the
type of the stream\ldots{}that means that we probably can't even know
what it type is. And that even if we could ``force'' it out somehow, we
would not even be able to work with it in a type-safe way!

In fact\ldots{}couldn't the state even \emph{{[}vary
dynamically{]}{[}wss{]}} as the stream progresses?
...
(All the code for Autos can be downloaded {[}from github{]}{[}autos{]}
and tried interactively online {[}at FPComplete{]}{[}autosint{]})
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"myStreamAuto ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"settableAuto ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\texttt{Nothing}, it'll give you \texttt{(\ n,\ counterFrom\ (n+1)\ )}.
If you give it \texttt{Just\ m}, it'll give you
\texttt{(\ m,\ counterFrom\ (m+1)\ )}.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"testAuto ::"} \StringTok{"testAuto_ ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"interactAuto ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"isEvenAuto ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\texttt{n} internally (in fact, like we said before, it is in theory
possible because we can't even know its type). It is completely sealed
off from the world, except by our explicit design. Here, we choose to
only ``offer'' a way to ``set'' it using our input.
...
of Streams. The type of an Auto (\texttt{Auto\ a\ b}) reveals the type
of the ``input'' and the ``ouput''\ldots{}but it never reveals nor fixes
the type of the ``state''. The type of the state is not only unknown, by
possibly dynamically changing over the course of the Auto's progression.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"summer ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\texttt{foldl} (technically, more like \texttt{scanl}): give us an
operator and an initial value, and we'll ``fold up'' all of our inputs.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"autoFold ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"accumulateIntoList ::"} \StringTok{"productor ::"} \StringTok{"accumulateStrings ::"} \StringTok{"monoidAccum ::"} \NormalTok{machines}
\end{Highlighting}
\end{Shaded}
...
{[}I've thrown up a few auto examples{]}{[}fewexamples{]}
...
!!{[}fewexamples{]}:machines/Auto.hs ``rollingAverage:'' ``onFor:''
``Command:'' ``autoMap:''
...
one. It internally holds a {[}Map{]}{[}{]} (a key-value store) --- you
can give it \texttt{{[}Command{]}{[}{]}} data types that tell it to
insert, lookup, and delete values. However, it enforces a maximum of
items.
...
!!{[}Command{]}:machines/Auto.hs ``Command:'' {[}Map{]}:
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{machines}\FunctionTok{/}\NormalTok{Auto.hs }\StringTok{"maybeIsEven ::"}
\end{Highlighting}
\end{Shaded}
...
\texttt{Auto\ ()\ b}. Streams are then ``function like things''
analogous to some \texttt{(-\textgreater{})\ ()\ b}, or
...
``asked for''. So, just like an Auto is a ``function'' that has memory,
...
with the revelation that the general class of these ``function things''
have a name! And maybe even\ldots{}a typeclass?
