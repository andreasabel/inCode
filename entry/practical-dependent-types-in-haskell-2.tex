...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L88-89}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L91-96}
...
matching on \texttt{ONet} gives yields the hidden existential (\texttt{hs}) and
also a type that is based on it (\texttt{Network\ i\ hs\ o}). It's like
\texttt{hs} ``paired'' with \texttt{Network\ i\ hs\ o}. Pattern match on the
results to give both the type (\texttt{hs}) \emph{and} the data structure. (If
we had implemented it as
...
familiar? In fact, you can even imagine that \texttt{OpaqueNet\ i\ o} as being
just a recursive \emph{Either} over \texttt{\textquotesingle{}{[}{]}},
\texttt{\textquotesingle{}{[}1{]}}, \texttt{\textquotesingle{}{[}1,2{]}},
etc.\footnote{A bit of a stretch, because the set of all \texttt{{[}Nat{]}}s is
  non-enumerable and uncountable, but you get the picture, right?}

And, remember that the basic way of handling an \texttt{Either} and figuring out
what the type of the value is inside is through \emph{pattern matching} on it.
You can't know if an \texttt{Either\ Int\ Bool} contains an \texttt{Int} or
...
\texttt{Sing\ hs}! (Or potentially, the network itself) But, once you pattern
match on it, all is revealed\ldots{}and GHC lets you take advantage of knowing
the type!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L60-67}
...
from people using the functionality externally.\footnote{This is a completely
  ...
  in the wild even offer \emph{both} styles in their external API.} A
\texttt{Sing\ a} is a normal Haskell value, but \texttt{SingI\ hs} is a
...
can create a \texttt{Network\ i\ hs\ o} to stuff into our \texttt{ONet}. And for
...
But because \emph{singletons} was implemented before the \emph{TypeInType}
extension in GHC 8, it has to be implemented with clunky ``Kind Proxies''. In a
future version of \emph{singletons}, they'll be implemented this way. Right now,
in the current system,
\texttt{SomeSing\ STrue\ ::\ SomeSing\ (KProxy\ ::\ KProxy\ Bool)}, and
\texttt{bar\ ::\ SomeSing\ (KProxy\ ::\ KProxy\ Nat)}.\footnote{Gross, right?
  ...
  debacle is now!} However, for the most part, the actual \emph{usage} of
\texttt{SomeSing}, so we can ignore this slight wart when we are actually
writing code.
...
singletons. It associates \texttt{Bool} the type with \texttt{Bool} the kind,
\texttt{Integer} the type with \texttt{Nat} the kind, \texttt{{[}Integer{]}} the
type with \texttt{{[}Nat{]}} the kind, etc., and it does it with straightforward
plane jane applications of type families --- here's a
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L98-102}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L157-163}
\OtherTok{main ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{main }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{putStrLn }\StringTok{"What hidden layer structure do you want?"}
...
what you write, the limitation of implementations, etc. \ldots{} all reasons any
...
\texttt{f} has to take a \texttt{Sing\ hs} and a \texttt{Network\ i\ hs\ o}, but
deal with it in a way that works \emph{for all} \texttt{hs}. It has to be:
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L125-125}
...
\texttt{Network\ i\ hs\ o} into an \texttt{OpaqueNet\textquotesingle{}\ i\ o\ r}
pretty straightforwardly:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L127-128}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- the skolemized `toSing`/`SomeSing`:}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L130-138}
\OtherTok{withRandomONet' ::} \NormalTok{(}\DataTypeTok{MonadRandom} \NormalTok{m, }\DataTypeTok{KnownNat} \NormalTok{i, }\DataTypeTok{KnownNat} \NormalTok{o)}
                \OtherTok{=>} \NormalTok{[}\DataTypeTok{Integer}\NormalTok{]}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L165-171}
\OtherTok{main' ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{main' }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{putStrLn }\StringTok{"What hidden layer structure do you want?"}
    \NormalTok{hs }\OtherTok{<-} \NormalTok{readLn}
...
\end{Shaded}
  ...
  is guilty}), so it can be useful for those, too!

  This is less useful for things like \texttt{toSing} where things are
...
\item
  ...
  \texttt{Binary} instances later for serialization/deserialization, and we'll
  need the wrapper for sure.
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L24-29}
...
\subsection{\texorpdfstring{Serializing
...
pretty nice because the entire structure is already known ahead of time:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L69-73}
...
\emph{just from the type}. If we want to deserialize/load a
...
\href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs\#L60-63}{\texttt{randomNet\textquotesingle{}}}:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L75-79}
...
We have to ``pattern match'' on \texttt{hs} using singletons to see what
constructor we are expecting to deserialize.
...
the arity/type of the function), so we have to switch to \texttt{SingI}-style
had have their \texttt{Binary} instances require a \texttt{SingI\ hs}
constraint.
...
It is interesting to note that \texttt{natVal} and \texttt{hiddenStruct} take
type-level information (\texttt{n}, \texttt{hs}) and turns them into term-level
values (\texttt{Integer}s, \texttt{{[}Integer{]}}s). In fact, they are kind of
the opposites of our reification functions like \texttt{toSing}. Going from the
``type level'' to the ``value level'' is known in Haskell as
\textbf{reflection}, and is the dual concept of reification. (The
\emph{singletons} library offers reflectors for all of its singletons, as
\texttt{fromSing}.)
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L104-109}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L111-116}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L118-120}
...
\item
  Implement
  \href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs\#L140-145}{\texttt{putONet\textquotesingle{}}}
  and
  \href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs\#L147-155}{\texttt{getONet\textquotesingle{}}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{!!!}\NormalTok{dependent}\FunctionTok{-}\NormalTok{haskell}\FunctionTok{/}\NormalTok{NetworkTyped2.hs }\StringTok{"data SomeNet"}
\end{Highlighting}
\end{Shaded}
  ...
  \tightlist
  \item
    \href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs\#L181-190}{\texttt{randomSNet}}
  \item
    The
    \href{https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs\#L192-206}{binary
...
\section{Dealing with Runtime Types}\label{dealing-with-runtime-types}
