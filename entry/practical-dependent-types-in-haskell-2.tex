...
I expanded out the type signature of \texttt{getONet\textquotesingle{}}, because
you'll see the explicit form more often. It's:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{getONet' ::} \NormalTok{(forall hs}\FunctionTok{.} \DataTypeTok{Sing} \NormalTok{hs }\OtherTok{->} \DataTypeTok{Network} \NormalTok{i hs o }\OtherTok{->} \DataTypeTok{Get} \NormalTok{r)}
         \OtherTok{->} \DataTypeTok{Get} \NormalTok{r}
\end{Highlighting}
\end{Shaded}

Which basically says ``Give what you would do if you \emph{had} a
\texttt{Sing\ hs} and a \texttt{Network\ i\ hs\ o}'', and I'll get them for you
