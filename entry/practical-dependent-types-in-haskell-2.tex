...
In fact, you can even imagine that \texttt{OpaqueNet\ i\ o} as being just a
recursive \texttt{Either} over \texttt{\textquotesingle{}{[}{]}},
\texttt{\textquotesingle{}{[}1{]}},
\texttt{\textquotesingle{}{[}1,2{]}\textquotesingle{}}, etc. (A bit of a
stretch, because the set of all \texttt{{[}Nat{]}}s is non-enumerable and
...
or \texttt{Bool} until you pattern match. But, once you do, all is revealed, and
...
pattern match on it, all is revealed\ldots{}and GHC lets you take advantage of
knowing the type!
...
(\texttt{toSing} works using a simple typeclass (``kindclass'', heh) mechanism
with associated types whose job is to associate \emph{value}'s types with the
kinds of their singletons. It associates \texttt{Bool} the type with
\texttt{Bool} the kind, \texttt{Integer} the type with \texttt{Nat} the kind,
\texttt{{[}Integer{]}} the type with \texttt{{[}Nat{]}} the kind, etc., and it
does it with straightforward plane jane applications of type families --- here's
a \href{https://ocharles.org.uk/blog/posts/2014-12-12-type-families.html}{nice
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L152-158}
\OtherTok{main ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{main }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{putStrLn }\StringTok{"What hidden layer structure do you want?"}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L142-150}
\OtherTok{withRandomONet' ::} \NormalTok{(}\DataTypeTok{MonadRandom} \NormalTok{m, }\DataTypeTok{KnownNat} \NormalTok{i, }\DataTypeTok{KnownNat} \NormalTok{o)}
                \OtherTok{=>} \NormalTok{[}\DataTypeTok{Integer}\NormalTok{]}
                \OtherTok{->} \NormalTok{(forall hs}\FunctionTok{.} \DataTypeTok{Sing} \NormalTok{hs }\OtherTok{->} \DataTypeTok{Network} \NormalTok{i hs o }\OtherTok{->} \NormalTok{m r)}
                \OtherTok{->} \NormalTok{m r}
...
for, as well\ldots{}and, in a way, it seems oddly more natural, in a way.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/dependent-haskell/NetworkTyped2.hs#L160-166}
\OtherTok{main' ::} \DataTypeTok{IO} \NormalTok{()}
\NormalTok{main' }\FunctionTok{=} \KeywordTok{do}
    \NormalTok{putStrLn }\StringTok{"What hidden layer structure do you want?"}
    \NormalTok{hs }\OtherTok{<-} \NormalTok{readLn}
