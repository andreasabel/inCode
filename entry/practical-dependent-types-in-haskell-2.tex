...
because \texttt{Sing}s are normal first-class values, unlike those global and
magical typeclasses) while at the same time removing the burden of calling with
explicit singletons from people using the functionality externally.\footnote{This
  is a completely personal style, and I can't claim to speak for all of the
  Haskell dependent typing community. In fact, I'm not even sure that you could
  even say that there is a consensus at all. But this is the style that has
  worked personally for me in both writing and using libraries! And hey, some
  libraries I've seen in the wild even offer \emph{both} styles in their
  external API.}
...
\ldots{} all reasons any Haskeller should be familiar with reciting.
...
We can try to write a \texttt{Binary} instance for \texttt{Network}. Of course,
we can't have \texttt{put} or \texttt{get} take a \texttt{Sing\ hs} input
(that'd change the arity/type of the function), so we have to switch to
\texttt{SingI}-style had have their \texttt{Binary} instances require a
\texttt{SingI\ hs} constraint.
...
It's arguably more useful to serialize \texttt{OpaqueNet}s, because between
different iterations, you might want to try out/store trained networks of
different internal structures. You can't even \emph{load} a \texttt{Network}
without knowing its internal structure. You want all your networks to be
mutually compatible and loadable\ldots{}so it makes more sense to
serialize/deserialize \texttt{OpaqueNet}s than \texttt{Network}s. You can load
them without knowing their internal structure, and you can keep them all in the
same list.
...
write a simple function to get the \texttt{{[}Integer{]}} of a network's
structure:
