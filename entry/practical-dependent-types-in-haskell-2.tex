...
\NormalTok{numHiddens }\FunctionTok{=} \NormalTok{\textbackslash{}}\KeywordTok{case} \DataTypeTok{ONet} \NormalTok{(}\OtherTok{ss ::} \DataTypeTok{Sing} \NormalTok{hs) _ }\OtherTok{->} \NormalTok{lengthSing ss}
  \KeywordTok{where}
...
singleton in, there'd be no way for us to recover the original \texttt{hs}! This
pattern is known as the \textbf{dependent pair}. Pair \texttt{hs} with some type
\texttt{f\ hs}, and pattern match on the \texttt{Sing\ hs} to reveal both!
...
Here's the key to making this all work: once you \emph{do} pattern match on
...
\subsubsection{Universal vs.~Existential}\label{universal-vs.existential}
...
And the function has to be implemented in a way that will work for \emph{any}
\texttt{a} and \texttt{b}. The function's implementation has the burden of being
