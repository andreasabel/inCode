...
\OtherTok{    ::}\NormalTok{ (}\DataTypeTok{Backprop}\NormalTok{ a, }\DataTypeTok{Backprop}\NormalTok{ s)}
    \OtherTok{=>} \DataTypeTok{ModelS}\NormalTok{ p s a a}
    \OtherTok{->}\NormalTok{ p}
    \OtherTok{->}\NormalTok{ s}
    \OtherTok{->}\NormalTok{ a}
    \OtherTok{->}\NormalTok{ [a]}
\NormalTok{feedback f p s0 x0 }\FunctionTok{=}\NormalTok{ unfoldr go (s0, x0)}
  \KeywordTok{where}
\NormalTok{    go (s, x) }\FunctionTok{=} \DataTypeTok{Just}\NormalTok{ (x, (s', y))}
      \KeywordTok{where}
