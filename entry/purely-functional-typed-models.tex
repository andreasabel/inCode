...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/functional-models/model.hs#L274-L285}
...
\NormalTok{ghci}\FunctionTok{>} \KeywordTok{let}\NormalTok{ output }\FunctionTok{=}\NormalTok{ feedback ar2 trained primed (series }\FunctionTok{!!} \DecValTok{19}\NormalTok{)}
\NormalTok{ghci}\FunctionTok{>}\NormalTok{ mapM_ print }\FunctionTok{$}\NormalTok{ take }\DecValTok{30}\NormalTok{ output}
\FunctionTok{-}\FloatTok{0.9980267284282716}
\FunctionTok{-}\FloatTok{0.9510565162972417}
\FunctionTok{-}\FloatTok{0.8443279255081759}
\FunctionTok{-}\FloatTok{0.6845471059406962}
\FunctionTok{-}\FloatTok{0.48175367412103653}
\FunctionTok{-}\FloatTok{0.24868988719256901}
\FunctionTok{-}\FloatTok{3.673766846290505e-11}
\FloatTok{0.24868988711894977}
\FloatTok{0.4817536740469978}
\FloatTok{0.6845471058659982}
\FloatTok{0.8443279254326351}
\FloatTok{0.9510565162207472}
\FloatTok{0.9980267283507953}
\FloatTok{0.9822872506502898}
\FloatTok{0.9048270523889208}
\FloatTok{0.7705132427021685}
\FloatTok{0.5877852522243431}
\FloatTok{0.3681245526237731}
\FloatTok{0.12533323351198067}
\FunctionTok{-}\FloatTok{0.1253332336071494}
\FunctionTok{-}\FloatTok{0.36812455271766376}
\FunctionTok{-}\FloatTok{0.5877852523157643}
\FunctionTok{-}\FloatTok{0.7705132427900961}
\FunctionTok{-}\FloatTok{0.9048270524725681}
\FunctionTok{-}\FloatTok{0.9822872507291605}
\FunctionTok{-}\FloatTok{0.9980267284247174}
\FunctionTok{-}\FloatTok{0.9510565162898851}
\FunctionTok{-}\FloatTok{0.844327925497479}
\FunctionTok{-}\FloatTok{0.6845471059273313}
\FunctionTok{-}\FloatTok{0.48175367410584324}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{(}\FunctionTok{-}\FloatTok{0.3276007378757607}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{9.49789925462907e-2}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.15326329240850092}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.4036006817890014}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.6365988256374424}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.8297644575358999}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.9644601077595601}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{1.0322337479560069}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{1.0354328415838387}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.98271304349553}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.8838820861246679}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.7469384572032421}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.5774599954294803}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.37953522246889254}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FloatTok{0.1576543900562724}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{8.079295377239147e-2}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.32316184922616614}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.5509792378000917}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.7428726769386842}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.8804772463971613}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.9537270792131795}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.9620288708442922}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.9114012854243098}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.8104104643872705}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\NormalTok{(}\FunctionTok{-}\FloatTok{0.6672968115106706}\OtherTok{ ::} \DataTypeTok{R} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
-0.96)\ldots{}but it still seems pretty nice considering that its memory units
are all sigmoidally squashed, while AR(2) gets to have a continuous memory
space. At this point we're picking hairs of 1\% difference, though! Sounds like
these RNNs have proven to be quite ``unreasonably effective'', eh?
