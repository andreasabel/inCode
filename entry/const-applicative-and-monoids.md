...
that has to use *both* inputs. After all, my earlier statement claimed that
`f <*> x` has to use both the effects of `f` and the effects of `x`, each
...
`<*>` and `pure ()`. In fact, it's a bit more than a monoid homomorphism -- it's
...
especially obvious demonstration of this. Hopefully this helps shed some
insight!
