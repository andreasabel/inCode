...
Essentially we're converting a \texttt{SomeDoor} into a \texttt{Sing\ s} and
\texttt{Door\ s}. We just can't know which \texttt{s}, a priori. So, we must
handle it with a
\texttt{forall\ s.\ Sing\ s\ -\textgreater{}\ Door\ s\ -\textgreater{}\ r}
(using \emph{RankNTypes}) -- a function that can handle \emph{any} \texttt{s}
...
\NormalTok{closeSomeOpenedDoor sd }\FunctionTok{=}\NormalTok{ withSomeDoor sd }\FunctionTok{$}\NormalTok{ \textbackslash{}}\KeywordTok{case}
    \DataTypeTok{SOpened} \OtherTok{->}\NormalTok{ \textbackslash{}d }\OtherTok{->} \DataTypeTok{Just} \FunctionTok{.}\NormalTok{ fromDoor_ }\FunctionTok{$}\NormalTok{ closeDoor d}
...
The secret ingredient here is the \texttt{Sing\ s} we store inside
\texttt{MkSomeDoor} -- it gives our pattern matchers the ability to deduce the
\texttt{s} type.
