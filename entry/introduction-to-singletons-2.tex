...
traces of it.\footnote{You might have noticed I was a bit sneaky by jumping
  straight \texttt{SomeDoor} when we already had a perfectly good ``I don't
  care'' option. We used it last post!
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door2.hs#L60-64}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door2.hs#L66-73}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/singletons/Door2.hs#L78-83}
\OtherTok{mkSomeDoor ::} \DataTypeTok{DoorState} \OtherTok{->} \DataTypeTok{String} \OtherTok{->} \DataTypeTok{SomeDoor}
...
\section{A Dependently Typed Outlook}\label{a-dependently-typed-outlook}
...
\emph{singletons} serves us today in that context.

At the end of the day, we really want to be able to write functions like:

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{lockAnyDoor ::}\NormalTok{ (}\OtherTok{s ::} \DataTypeTok{DoorState}\NormalTok{) }\OtherTok{->} \DataTypeTok{Door}\NormalTok{ s }\OtherTok{->} \DataTypeTok{Door} \CharTok{'Locked}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{lockAnyDoor ::} \DataTypeTok{Sing}\NormalTok{ (}\OtherTok{s ::} \DataTypeTok{DoorState}\NormalTok{) }\OtherTok{->} \DataTypeTok{Door}\NormalTok{ s }\OtherTok{->} \DataTypeTok{Door} \CharTok{'Locked}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{loadDoor ::}\NormalTok{ FilePath }\OtherTok{->} \DataTypeTok{IO}\NormalTok{ (s }\FunctionTok{**} \DataTypeTok{Door}\NormalTok{ s)}
\end{Highlighting}
\end{Shaded}

Where the \texttt{(s\ **\ Door\ s)} dependent tuple notation signifies an
existentially quantified \texttt{s} type that is chosen by \texttt{loadDoor} at
runtime.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{SomeDoor} \FunctionTok{=}\NormalTok{ forall s}\FunctionTok{.} \DataTypeTok{SomeDoor}\NormalTok{ (}\DataTypeTok{Sing}\NormalTok{ s) (}\DataTypeTok{Door}\NormalTok{ s)}
\OtherTok{loadDoor ::}\NormalTok{ FilePath }\OtherTok{->} \DataTypeTok{IO} \DataTypeTok{SomeDoor}

\NormalTok{withLoadedDoor}
\OtherTok{    ::}\NormalTok{ FilePath}
    \OtherTok{->}\NormalTok{ (forall s}\FunctionTok{.} \DataTypeTok{Sing}\NormalTok{ s }\OtherTok{->} \DataTypeTok{Door}\NormalTok{ s }\OtherTok{->} \DataTypeTok{IO}\NormalTok{ r) }\OtherTok{->} \DataTypeTok{IO}\NormalTok{ r}
\end{Highlighting}
\end{Shaded}

I mention this here before moving on to remind you of the ``big picture'' of
\emph{why} singletons is important in the current state of Haskell. We really
\emph{want} to be able to pattern match and branch on types, and we really
\emph{want} to be able to encode existential types using the quantified type
directly. However, ``until then'', we have \texttt{Sing\ s} instead of
\texttt{s}, and \texttt{forall\ s.\ SomeDoor\ (Sing\ s)\ (Door\ s)}, instead of
\texttt{s\ **\ Door\ s}.
