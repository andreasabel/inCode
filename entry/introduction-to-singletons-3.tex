...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{knock ::}\NormalTok{ (}\DataTypeTok{StatePass}\NormalTok{ s }\FunctionTok{~}\NormalTok{ '}\DataTypeTok{Allow}\NormalTok{) }\OtherTok{=>} \DataTypeTok{Door}\NormalTok{ s }\OtherTok{->} \DataTypeTok{IO}\NormalTok{ ()}
\NormalTok{knock d }\FunctionTok{=}\NormalTok{ putStrLn }\FunctionTok{$} \StringTok{"Knock knock on "} \FunctionTok{++}\NormalTok{ doorMaterial d }\FunctionTok{++} \StringTok{" door!"}
\end{Highlighting}
\end{Shaded}
...
obstruct, or partial. In that case, we can restrict a function based on the pass
state being equal to any of the three or more. Using the dependently typed proof
...
mix the two. (And, in fact, the constraint
\texttt{StatePass\ s\ \textasciitilde{}\ \textquotesingle{}Allow} could just be
replaced with asking for the proof of the predicate
\texttt{StatePass\ s\ :\textasciitilde{}:\ \textquotesingle{}Allow}). It's good
to understand the trade-offs in up-front cost, expressiveness, and correctness!
But, however way you play, the \emph{singletons} library is here to make our
life easier.
...
\subsubsection{Automatically Promoting
