...
\NormalTok{knockSomeDoor (}\DataTypeTok{MkSomeDoor}\NormalTok{ s d) }\FunctionTok{=} \KeywordTok{case}\NormalTok{ sStatePass s }\KeywordTok{of}
    \DataTypeTok{SObstruct} \OtherTok{->}\NormalTok{ knock d                         }\CommentTok{-- ^ `StatePass s ~ 'Obstruct`}
...
\item
  ...
  \texttt{Sing\ (s\ ::\ DoorState)}) What ``classical
...
\end{Shaded}
  ...
  the past? Maybe, perhaps, similiar to types that are a part of basic beginner
  Haskell concepts?
\item
  \texttt{And\ p\ q} and \texttt{Or\ p\ q} are decidable if \texttt{p} and
  \texttt{q} are. Can we write the decision functions?

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{decideAnd}
\OtherTok{    ::}\NormalTok{ (forall x}\FunctionTok{.} \DataTypeTok{Sing}\NormalTok{ x }\OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (p x)}
    \OtherTok{->}\NormalTok{ (forall x}\FunctionTok{.} \DataTypeTok{Sing}\NormalTok{ x }\OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (q x)}
    \OtherTok{->} \DataTypeTok{Sing}\NormalTok{ a}
    \OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (}\DataTypeTok{And}\NormalTok{ p q a)}

\NormalTok{decideOr}
\OtherTok{    ::}\NormalTok{ (forall x}\FunctionTok{.} \DataTypeTok{Sing}\NormalTok{ x }\OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (p x)}
    \OtherTok{->}\NormalTok{ (forall x}\FunctionTok{.} \DataTypeTok{Sing}\NormalTok{ x }\OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (q x)}
    \OtherTok{->} \DataTypeTok{Sing}\NormalTok{ a}
    \OtherTok{->} \DataTypeTok{Decide}\NormalTok{ (}\DataTypeTok{Or}\NormalTok{ p q a)}
\end{Highlighting}
\end{Shaded}

  This question requires definitions for \texttt{And} and \texttt{Or}. If you
  skipped the previous question, check out the definitions I have included in
  the solutions!
\item
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{to   ::} \DataTypeTok{SingI}\NormalTok{ s }\OtherTok{=>} \DataTypeTok{Knockable}\NormalTok{ s             }\OtherTok{->} \DataTypeTok{Refuted}\NormalTok{ (s }\FunctionTok{:~:}\NormalTok{ '}\DataTypeTok{Opened}\NormalTok{)}
\NormalTok{to   }\FunctionTok{=} \CommentTok{-- ???}

\OtherTok{from ::} \DataTypeTok{SingI}\NormalTok{ s }\OtherTok{=>} \DataTypeTok{Refuted}\NormalTok{ (s }\FunctionTok{:~:}\NormalTok{ '}\DataTypeTok{Opened}\NormalTok{) }\OtherTok{->} \DataTypeTok{Knockable}\NormalTok{ s}
\NormalTok{from }\FunctionTok{=} \CommentTok{-- ???}
\end{Highlighting}
\end{Shaded}
\item
...
\NormalTok{knockRefl d }\FunctionTok{=}\NormalTok{ putStrLn }\FunctionTok{$} \StringTok{"Knock knock on "} \FunctionTok{++}\NormalTok{ doorMaterial d }\FunctionTok{++} \StringTok{" door!"}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::} \DataTypeTok{SomeDoor}
    \OtherTok{->} \DataTypeTok{IO}\NormalTok{ ()}
\NormalTok{knockSomeDoorRefl }\FunctionTok{=} \CommentTok{-- ??}
\end{Highlighting}
\end{Shaded}

  Remember not to use \texttt{knock}.

  Assume that \texttt{DoorState} has an instance of \texttt{SDecide}. This
  should be derived automatically as long as you derive \texttt{Eq}:
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{invertPass ::} \DataTypeTok{Pass} \OtherTok{->} \DataTypeTok{Pass}
\NormalTok{invertPass }\DataTypeTok{Obstruct} \FunctionTok{=} \DataTypeTok{Allow}
\NormalTok{invertPass }\DataTypeTok{Allow}    \FunctionTok{=} \DataTypeTok{Obstruct}
\end{Highlighting}
\end{Shaded}
...
\OtherTok{    ::} \DataTypeTok{SomeDoor}
    \OtherTok{->} \DataTypeTok{IO}\NormalTok{ ()}
\NormalTok{knockSomeDoorInv }\FunctionTok{=} \CommentTok{-- ??}
\end{Highlighting}
\end{Shaded}
\item
...
\end{Shaded}
  ...
  Manually promote this instance for \texttt{DoorState} to the type level:
