...
as a value:\footnote{Really, we could just use \texttt{Bool} instead of defining
  a \texttt{Pass} type. We're just going through a new type for the sake of
  example, and it can be useful because a type like \texttt{Pass} might
  potentially have even more constructors!}
...
At the high level, we defined a ``function'' on types (\texttt{PassState}),
using type families.
...
have to define \emph{singleton functions} (\texttt{sPassState}) at the value
...
tedious and mechanical process. This is where the \emph{singletons} library
...
\NormalTok{  passState :: DoorState -> Pass}
\NormalTok{  passState Opened = Allow}
\NormalTok{  passState Closed = Obstruct}
\NormalTok{  passState Locked = Obstruct}
\NormalTok{  |])}
\end{Highlighting}
\end{Shaded}
...
\item
  The value-level function \texttt{passState} with the type
...
However, with singleton's template haskell, this also generates:\footnote{In the
  ...
  things (known as defunctionalization symbols), which we will be talking about
...
\item
  The \emph{type family} \texttt{PassState\ (s\ ::\ DoorState)\ ::\ Pass}, like
  we defined above
\item
  The \emph{singleton function}
  \texttt{sPassState\ ::\ Sing\ s\ -\textgreater{}\ Sing\ (PassState\ s)}, like
  we defined above.
\end{enumerate}
...
identical name) and the singleton function \texttt{\%++}.
