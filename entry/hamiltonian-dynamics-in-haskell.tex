...
dimension, \(KE(\dot{x}) = \frac{1}{2} m \dot{x}^2\). So the momentum conjugate
to \(x\) is:
...
it a name, \(\hat{P}\). \(\hat{P}\) is a symmetric matrix (by construction), and
it's important to remember that it's an explicit function of \(\mathbf{q}\)
(because \(\hat{J}_f\) is a matrix of partial derivatives at a given
\(\mathbf{q}\)). So, \(\mathbf{p} = \hat{P} \dot{\mathbf{q}}\)
