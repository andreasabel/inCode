...
it a name, \(\hat{T}\). If the masses are all positive and \(\hat{J}_f\) is
full-rank, then \(\hat{T}\) is a symmetric, positive-definite, invertible matrix
...
vector of conjugate momenta, then: \(\mathbf{p} = \hat{T} \dot{\mathbf{q}}\)
