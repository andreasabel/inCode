...
can write this in a nicer by abusing matrix multiplication notation as:
...
\hat{K}\^{}\{-1\} \hat{J}\_f\^{}T \hat{M} \hat{H}\emph{f\left{[}
\nabla}\{\mathbf{q}\} \hat{J}\emph{f \righo \hat{K}\^{}\{-1\} \mathbf{p} +
\nabla}\{\mathbf{q}\} PE(\mathbf{q}) ``)
...
Finally, we can get our Hessian Tensor by using \texttt{hessianF}:\footnote{\texttt{hessian}
  computes the Hessian Matrix for a \$\mathbf{R}\^{}n
  \rightarrow \includegraphics{https://latex.codecogs.com/png.latex?\%5Cmathbf\%7BR\%7D}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/hamilton1/Hamilton.hs#L174-178}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{-- source: https://github.com/mstksg/inCode/tree/master/code-samples/hamilton1/Hamilton.hs#L181-210}
