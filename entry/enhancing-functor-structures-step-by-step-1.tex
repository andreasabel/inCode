...
\hypertarget{interpreting-parsers}{%
\subsection{Interpreting Parsers}\label{interpreting-parsers}}
...
illustrates the duality between sum types, since \texttt{Ap} and \texttt{ListF}
...
\emph{aeson-better-errors} doesn't provide and \texttt{Plus} instances for
\texttt{Parse}. We can write them as orphans here just because this is a fun
learning experience (but we usually do like to avoid defining instances for
types or typeclasses that aren't ours).
...
So our \texttt{ListF\ Choice\ a} is just \texttt{{[}Choice\ a{]}}. This is
