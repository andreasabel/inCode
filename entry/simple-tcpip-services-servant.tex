...
going to teach you \emph{how} to write a todo app, but rather how to hook up a
todo app over a TCP/IP connection quickly. This post can also serve as a
stepping-stone to a ``microservices architecture'', if you intend to build
towards one (this is explored deeper by {[}k-bx{]}{[}owlcloud{]})\ldots but
really it's more focused for standalone user-facing applications. How you apply
these techniques is up to you :)

This article is written for the late beginner to intermediate haskeller, who
knows how to ``do'' what they want the server to do, but only just needs a
simple way to hook it up over a TCP/IP connection.
...
\emph{{[}client-cli{]}{[}{]}} to automatically generate an
\emph{optparse-applicative}-based command line interface that allows you to
directly specify your commands based on command line arguments
...
\NormalTok{    manager\textquotesingle{} }\OtherTok{<{-}}\NormalTok{ newManager defaultManagerSettings}
\NormalTok{    res      }\OtherTok{<{-}}\NormalTok{ runClientM c }\OperatorTok{$}
\NormalTok{      mkClientEnv manager\textquotesingle{} (}\DataTypeTok{BaseUrl} \DataTypeTok{Http} \StringTok{"localhost"} \DecValTok{3434} \StringTok{""}\NormalTok{)}
...
\item
  ...
  handle them each into any type we want (even \texttt{IO}) as long as each
  handler returns something of the same type.
...
quickly and effectively drill down every option.
