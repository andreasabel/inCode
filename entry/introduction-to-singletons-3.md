...
For those unfamiliar with the `a -> Void` idiom, `a -> Void` is a type we use in
Haskell to represent the fact that it is impossible to construct a value of type
`a`. That's because if you could, then you could give it to an `a -> Void` to
get a value of type `Void`, which is impossible to have.
